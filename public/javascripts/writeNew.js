...
YAHOO.register("element", YAHOO.util.Element, { "version": "2.5.0", "build": "897" });
...
{
  var D = new deconcept.SWFObject(J, E, "100%", "100%", C, F);
  if (G)
    {
...
YAHOO.widget.Uploader = function (A, B)
{
  var C = "window";
  if (!B)
    {
...
if (!YAHOO.util.DragDropMgr)
  {
    YAHOO.util.DragDropMgr = function ()
    {
      ...
      }, "_onLoad": function ()
      {
        ...
        A.on(window, "resize", this._onUnloadthis, true);
        ...
        for (R = 0; R < D.length; ++R)
          {
            ...
            if (X && X.length)
              {
                var G = D[R].charAt(0).toUpperCase() + D[R].substr(1), W = "onDrag" + G, I = "b4Drag" + G, N = "drag" + G + "Event", V = "drag" + G;
                if (this.mode)
                  {
                    if (Z.events[I])
                      {
      ...
      {
        var D, B;
        var E = document.documentElement, C = document.body;
...
(function ()
{
  ...
  {
    var P;
    var N = this.getItems(), Q = N.length;
    for (var O = 0; O < Q; ++O)
      {
        P = N[O];
  ...
  {
    var T, S, N;
    var P = typeof Q == "number" ? Q : 0, R = this._getItemGroup(P);
    if (R)
      {
        T = R.splice(O, 1);
        S = T[0];
  ...
  {
    var R, Q, N;
    var P = this._getItemGroup(O), S = P.length;
    if (S > 0)
      {
        N = S - 1;
        do 
  ...
  }, "_onMouseOver": function (W, R)
  {
    ...
    var X = R[0], V = R[1], N = M.getTarget(X);
  ...
  }, "_onMouseOut": function (V, P)
  {
    ...
    var W = P[0], T = P[1], Q = M.getRelatedTarget(W), U = false;
  ...
  {
    var R, O, N, S, U, X;
    var W = YAHOO.util.Event, P = YAHOO.util.Dom, Z = Q[0], T = Q[1], V = false;
    if (T)
      {
  ...
  {
    var U, Z, O, S, c, N, e, R, a, Q, W, d, T;
    var Y = V[0], X = V[1], f = this;
    function P()
    {
  ...
  }, "_onScrollTargetMouseOver": function (T, W)
  {
    var N, O;
    this._cancelHideDelay();
    var P = M.getTarget(T), R = this.body, V = this, Q = this.cfg.getProperty("scrollincrement");
    function U()
    {
      var X = R.scrollTop;
      if (X < N)
  ...
  {
    var N, S;
    var R = this.element, U = this._aListElements.length, O = true, T = 0;
    if (U > 0)
      {
        do 
          {
  ...
  {
    var Y, h, O, N, g, e, V, U;
    var j = this.parent, f = B.VIEWPORT_OFFSET, b = this.element, S = this.cfg, T = P[0], R = b.offsetHeight, c = b.offsetWidth, i = C.getViewportWidth(), a = C.getViewportHeight(), Z = j && j.parent instanceof YAHOO.widget.MenuBar ? 0 : f, d = S.getProperty("context"), X = d ? d[0] : null;
    if (c < i)
      {
        V = T[0];
  ...
  {
    var N;
    var R = this.element, Q = O[0] == "static" ? "static" : "absolute", T = this.cfg;
  ...
  {
    var V, S, P;
    var T = U[0], Q = this.element, R = this.body, Y = this.header, O = this.footer, W = this._onScrollTargetMouseOver, b = this._onScrollTargetMouseOut, N = this.cfg.getProperty("minscrollheight");
    if (T !== 0 && T < N)
      {
        T = N;
      }
  ...
  {
    var N, P, R, Q;
    var O = this.getItems(), S = O.length;
    if (S > 0)
      {
        N = [];
        for (Q = 0; Q < S; ++Q)
  ...
  {
    var U, T, S;
    var R = this.getItems(), O = R.length, P = this.element, Q = this.body, V = this.header, N = this.footer;
    if (O > 0)
      {
        S = O - 1;
        do 
...
(function ()
{
  ...
  {
    var N, M, K, J, O;
    var P = this.srcElement, L = this.cfg;
  ...
  {
    var U, W, M;
    var Q = O[0], P = this.cfg, K = this.element, T = this._oAnchor, N = this.parent && this.parent.lazyLoad, J = "-hassubmenu", L = this.CSS_CLASS_NAME + J, S = this.CSS_LABEL_CLASS_NAME + J;
    if (Q)
      {
        if (Q instanceof E)
          {
...
(function ()
{
  var G = YAHOO.util.Dom, L = YAHOO.util.Event, I = YAHOO.lang, B = YAHOO.widget.Overlay, J = YAHOO.widget.Menu, D = { }, K = null, E = null, C = null;
  function F(N, M, Q, O)
  {
    var R, P;
    if (I.isString(N) && I.isString(M))
      {
        if (YAHOO.env.ua.ie)
          {
            P = "<input type=\"" + N + "\" name=\"" + M + "\"";
            if (O)
              {
                P += " checked";
              }
  ...
  {
    var S, O, P;
    var M = N.nodeName.toUpperCase(), R = this;
    function U(V)
    {
      if (!(V in T))
        {
          S = N.getAttributeNode(V);
          if (S && "value" in S)
            {
              T[V] = S.value;
            }
    ...
    switch (M) {
      ...
      case "BUTTON":
        ...
        if (G.hasClass(O, this.CSS_CLASS_NAME + "-disabled"))
          {
            T.disabled = true;
          }
  ...
  function A(O)
  {
    var N = O.attributes, M = N.srcelement, Q = M.nodeName.toUpperCase(), P = this;
    if (Q == this.NODE_NAME)
      {
        O.element = M;
        O.id = M.id;
        G.getElementsBy(function (R)
        {
          switch (R.nodeName.toUpperCase()) {
            case "BUTTON":
            case "A":
            case "INPUT":
              H.call(P, R, N);
  ...
  YAHOO.widget.Button = function (Q, N)
  {
    var O, M;
    ...
    var P = YAHOO.widget.Button.superclass.constructor;
    if (arguments.length == 1 && !I.isString(Q) && !Q.nodeName)
      {
        if (!Q.id)
          {
            Q.id = G.generateId();
          }
        P.call(this, this.createButtonElement(Q.type), Q);
      }
    else
      {
        O = { "element": null, "attributes": N || { } };
        if (I.isString(Q))
          {
            M = G.get(Q);
            if (M)
              {
                ...
                O.attributes.srcelement = M;
                A.call(this, O);
                if (!O.element)
                  {
                    O.element = this.createButtonElement(O.attributes.type);
                  }
                P.call(this, O.element, O.attributes);
              }
          }
        else
          {
                ...
                if (!O.attributes.id)
                  {
                    if (Q.id)
                      {
                        O.attributes.id = Q.id;
                      }
                    else
                      {
                        O.attributes.id = G.generateId();
                      }
                  }
                O.attributes.srcelement = Q;
                A.call(this, O);
                if (!O.element)
                  {
                    O.element = this.createButtonElement(O.attributes.type);
  ...
  }, "_setTabIndex": function (M)
  {
    this._button.tabIndex = M;
    ...
    if (this.get("type") != "link")
      {
        if (!M)
          {
            switch (this.get("type")) {
              case "radio":
                M = this.RADIO_DEFAULT_TITLE;
                break;
              case "checkbox":
                M = this.CHECKBOX_DEFAULT_TITLE;
                break;
              case "menu":
                M = this.MENUBUTTON_DEFAULT_TITLE;
                break;
              case "split":
                M = this.SPLITBUTTON_DEFAULT_TITLE;
                break;
              case "submit":
                M = this.SUBMIT_TITLE;
                break;
        ...
        this._button.title = M;
      }
  }, "_setDisabled": function (M)
  {
    if (this.get("type") != "link")
      {
        if (M)
  ...
  }, "_setTarget": function (M)
  {
    if (this.get("type") == "link")
      {
        this._button.setAttribute("target", M);
      }
        ...
        if (N)
          {
            ...
            M = O == "radio" ? this.RADIO_CHECKED_TITLE : this.CHECKBOX_CHECKED_TITLE;
          }
        else
          {
            this.removeStateCSSClasses("checked");
            M = O == "radio" ? this.RADIO_DEFAULT_TITLE : this.CHECKBOX_DEFAULT_TITLE;
          }
        this.set("title", M);
      }
  }, "_setMenu": function (W)
  {
    var M, Z, P, S, O, N, V, R;
    var Q = this.get("lazyloadmenu"), T = this.get("element"), Y = false;
    if (!B)
      {
        return false;
      }
    if (J)
      {
        M = J.prototype.CSS_CLASS_NAME;
      }
    function X()
    {
      Z.render(T.parentNode);
      this.removeListener("appendTo", X);
    }
    function U()
    {
      if (Z)
        {
          G.addClass(Z.element, this.get("menuclassname"));
          G.addClass(Z.element, "yui-" + this.get("type") + "-button-menu");
          Z.showEvent.subscribe(this._onMenuShow, null, this);
          Z.hideEvent.subscribe(this._onMenuHide, null, this);
          Z.renderEvent.subscribe(this._onMenuRender, null, this);
          if (J && Z instanceof J)
            {
              Z.keyDownEvent.subscribe(this._onMenuKeyDown, this, true);
              Z.subscribe("click", this._onMenuClick, this, true);
              Z.itemAddedEvent.subscribe(this._onMenuItemAdded, this, true);
              S = Z.srcElement;
              if (S && S.nodeName.toUpperCase() == "SELECT")
                {
          ...
          if (!Y)
            {
              if (Q && J && !(Z instanceof J))
                {
                  Z.beforeShowEvent.subscribe(this._onOverlayBeforeShow, null, this);
                }
              else
                {
                  if (!Q)
    ...
    if (W && J && W instanceof J)
            ...
            do 
              {
                ...
                if (V)
                  {
                    V.cfg.subscribeToConfigEvent("selected", this._onMenuItemSelected, V, this);
                  }
              }
            while (R--);
          }
      ...
      {
        if (B && W && W instanceof B)
          {
            Z = W;
            Y = true;
            Z.cfg.setProperty("visible", false);
            Z.cfg.setProperty("context", [T,"tl","bl"]);
            U.call(this);
          }
        else
          {
            if (J && I.isArray(W))
              {
                ...
                {
                  Z = new J(G.generateId(), { "lazyload": Q, "itemdata": W });
                  U.call(this);
                });
              }
            else
                ...
                if (I.isString(W))
                  {
                    ...
                    if (P)
                      {
                        if (J && G.hasClass(P, M) || P.nodeName.toUpperCase() == "SELECT")
                          {
                            Z = new J(W, { "lazyload": Q });
                            U.call(this);
                          }
                        else
                          {
                            if (B)
                              {
                                Z = new B(W, { "visible": false, "context": [T,"tl","bl"] });
                                U.call(this);
                              }
                          }
                      }
                ...
                else
                  {
                    if (W && W.nodeName)
                      {
                        if (J && G.hasClass(W, M) || W.nodeName.toUpperCase() == "SELECT")
                          {
                            Z = new J(W, { "lazyload": Q });
                            U.call(this);
                          }
  ...
  }, "_setOnClick": function (M)
  {
    if (this._onclickAttributeValue && this._onclickAttributeValue != M)
      {
        this.removeListener("click", this._onclickAttributeValue.fn);
        this._onclickAttributeValue = null;
      }
    if (!this._onclickAttributeValue && I.isObject(M) && I.isFunction(M.fn))
      {
        this.on("click", M.fn, M.obj, M.scope);
        this._onclickAttributeValue = M;
      }
  }, "_setSelectedMenuItem": function (N)
  {
    var O;
    var M = this._menu;
    if (J && M && M instanceof J)
      {
        O = M.getItem(N);
        if (O && !O.cfg.getProperty("selected"))
          {
            O.cfg.setProperty("selected", true);
          }
      }
  }, "_isActivationKey": function (M)
  {
    var O;
    var Q = this.get("type"), N = Q == "checkbox" || Q == "radio" ? this.CHECK_ACTIVATION_KEYS : this.ACTIVATION_KEYS, P = N.length;
    if (P > 0)
      {
        O = P - 1;
        do 
  ...
  }, "_isSplitButtonOptionKey": function (M)
  {
    return M.ctrlKey && M.shiftKey && L.getCharCode(M) == 77;
  }, "_addListenersToForm": function ()
  {
    var Q, M, P, O, N;
    var S = this.getForm(), R = YAHOO.widget.Button.onFormKeyPress;
    if (S)
      {
        L.on(S, "reset", this._onFormReset, null, this);
        L.on(S, "submit", this.createHiddenFields, null, this);
        M = this.get("srcelement");
        if (this.get("type") == "submit" || M && M.type == "submit")
          {
            P = L.getListeners(S, "keypress");
            Q = false;
            if (P)
              {
                O = P.length;
  ...
  }, "_showMenu": function (R)
  {
    ...
    var P = B.VIEWPORT_OFFSET, Y = this._menu, W = this, Z = W.get("element"), T = false, V = G.getY(Z), U = G.getDocumentScrollTop();
    if (U)
      {
        V = V - U;
      }
    var O = V, N = G.getViewportHeight() - (V + Z.offsetHeight);
    function S()
    {
      if (T)
        {
    ...
    function a()
    {
      ...
      if (Q > c)
        {
          M = Y.cfg.getProperty("minscrollheight");
          if (c > M)
            {
              Y.cfg.setProperty("maxheight", c);
              if (T)
                {
                  Y.align("bl", "tl");
                }
          ...
          if (c < M)
            {
              if (T)
                {
                  Y.cfg.setProperty("context", [Z,"tl","bl"], true);
                  Y.align("tl", "bl");
                }
              else
                {
                  Y.cfg.setProperty("context", [Z,"bl","tl"], true);
                  Y.align("bl", "tl");
                  T = true;
                  return a();
                }
    ...
    if (J && Y && Y instanceof J)
      {
        Y.cfg.applyConfig({ "context": [Z,"tl","bl"], "clicktohide": false, "visible": true });
        Y.cfg.fireQueue();
        Y.cfg.setProperty("maxheight", 0);
        Y.align("tl", "bl");
        if (R.type == "mousedown")
          {
            L.stopPropagation(R);
          }
        Q = Y.element.offsetHeight;
        b = Y.element.lastChild;
        a();
  ...
  }, "_onMouseOver": function (M)
  {
    ...
    if (this._activationButtonPressed || this._bOptionPressed)
      {
        L.removeListener(document, "mouseup", this._onDocumentMouseUp);
      }
  }, "_onMouseOut": function (M)
  {
    ...
    if (this._activationButtonPressed || this._bOptionPressed)
      {
        L.on(document, "mouseup", this._onDocumentMouseUp, null, this);
      }
  }, "_onDocumentMouseUp": function (O)
  {
    var M, N;
    ...
    if (P == "menu" || P == "split")
      {
        M = L.getTarget(O);
        N = this._menu.element;
        if (M != N && !G.isAncestor(N, M))
          {
            this.removeStateCSSClasses(P == "menu" ? "active" : "activeoption");
            this._hideMenu();
          }
      }
    L.removeListener(document, "mouseup", this._onDocumentMouseUp);
    ...
    if ((P.which || P.button) == 1)
      {
        ...
        if (R == "split")
          {
            N = this.get("element");
            Q = L.getPageX(P) - G.getX(N);
            if (N.offsetWidth - this.OPTION_AREA_WIDTH < Q)
              {
  ...
  }, "_onFocus": function (N)
  {
    ...
    if (!this._hasKeyEventHandlers)
      {
        M = this._button;
        L.on(M, "blur", this._onBlur, null, this);
        L.on(M, "keydown", this._onKeyDown, null, this);
        L.on(M, "keyup", this._onKeyUp, null, this);
  ...
  }, "_onBlur": function (M)
  {
    ...
    C = null;
    this.fireEvent("blur", M);
  }, "_onDocumentKeyUp": function (M)
  {
    if (this._isActivationKey(L.getCharCode(M)))
      {
        this._activationKeyPressed = false;
        L.removeListener(document, "keyup", this._onDocumentKeyUp);
      }
  }, "_onKeyDown": function (N)
  {
    var M = this._menu;
    if (this.get("type") == "split" && this._isSplitButtonOptionKey(N))
      {
        this.fireEvent("option", N);
      }
    else
      {
        if (this._isActivationKey(L.getCharCode(N)))
          {
            if (this.get("type") == "menu")
              {
                this._showMenu(N);
    ...
    if (M && M.cfg.getProperty("visible") && L.getCharCode(N) == 27)
      {
        M.hide();
        this.focus();
      }
  ...
  }, "_onClick": function (P)
  {
    ...
    switch (S) {
      ...
      case "checkbox":
        if (this.get("checked"))
          {
            M = S == "radio" ? this.RADIO_CHECKED_TITLE : this.CHECKBOX_CHECKED_TITLE;
          }
        else
          {
            M = S == "radio" ? this.RADIO_DEFAULT_TITLE : this.CHECKBOX_DEFAULT_TITLE;
          }
        this.set("title", M);
        break;
      case "submit":
      ...
      case "reset":
        ...
        break;
      case "menu":
        M = this._menu.cfg.getProperty("visible") ? this.MENUBUTTON_MENU_VISIBLE_TITLE : this.MENUBUTTON_DEFAULT_TITLE;
        this.set("title", M);
        break;
      case "split":
        O = this.get("element");
        R = L.getPageX(P) - G.getX(O);
        ...
        M = this._menu.cfg.getProperty("visible") ? this.SPLITBUTTON_OPTION_VISIBLE_TITLE : this.SPLITBUTTON_DEFAULT_TITLE;
        this.set("title", M);
        break;
    }
  }, "_onAppendTo": function (N)
  {
    ...
    window.setTimeout(function ()
    {
      M._addListenersToForm();
    }, 0);
  }, "_onFormReset": function (N)
  {
    var O = this.get("type"), M = this._menu;
    if (O == "checkbox" || O == "radio")
      {
  ...
  }, "_onDocumentMouseDown": function (P)
  {
    var M = L.getTarget(P), O = this.get("element"), N = this._menu.element;
    if (M != O && !G.isAncestor(O, M) && M != N && !G.isAncestor(N, M))
      {
        this._hideMenu();
        L.removeListener(document, "mousedown", this._onDocumentMouseDown);
      }
  }, "_onOption": function (M)
  {
  ...
  }, "_onOverlayBeforeShow": function (N)
  {
    var M = this._menu;
    M.render(this.get("element").parentNode);
    M.beforeShowEvent.unsubscribe(this._onOverlayBeforeShow);
  }, "_onMenuShow": function (N)
  {
    var M, O;
    L.on(document, "mousedown", this._onDocumentMouseDown, null, this);
    if (this.get("type") == "split")
      {
        M = this.SPLITBUTTON_OPTION_VISIBLE_TITLE;
        O = "activeoption";
      }
    else
      {
        M = this.MENUBUTTON_MENU_VISIBLE_TITLE;
        O = "active";
      }
    this.addStateCSSClasses(O);
    this.set("title", M);
  }, "_onMenuHide": function (O)
  {
    var M, P;
    var N = this._menu;
    if (this.get("type") == "split")
    ...
    this.removeStateCSSClasses(P);
    this.set("title", M);
    if (this.get("type") == "split")
      {
        this._bOptionPressed = false;
      }
  }, "_onMenuKeyDown": function (O, N)
  {
    var M = N[0];
    if (L.getCharCode(M) == 27)
      {
        this.focus();
        if (this.get("type") == "split")
          {
  ...
  }, "_onMenuRender": function (N)
  {
    var P = this.get("element"), M = P.parentNode, O = this._menu.element;
    if (M != O.parentNode)
      {
        M.appendChild(O);
      }
    this.set("selectedMenuItem", this.get("selectedMenuItem"));
  }, "_onMenuItemSelected": function (O, N, M)
  {
    var P = N[0];
    if (P)
      {
        this.set("selectedMenuItem", M);
      }
  }, "_onMenuItemAdded": function (O, N, M)
  {
    var P = N[0];
    P.cfg.subscribeToConfigEvent("selected", this._onMenuItemSelected, P, this);
  }, "_onMenuClick": function (N, M)
  {
    var O;
    var P = M[1];
    if (P)
  ...
  }, "createButtonElement": function (M)
  {
    var O = this.NODE_NAME, N = document.createElement(O);
    N.innerHTML = "<" + O + " class=\"first-child\">" + (M == "link" ? "<a></a>" : "<button type=\"button\"></button>") + "</" + O + ">";
    return N;
  }, "addStateCSSClasses": function (M)
  {
    var N = this.get("type");
    if (I.isString(M))
      {
        if (M != "activeoption")
          {
            this.addClass(this.CSS_CLASS_NAME + ("-" + M));
          }
        this.addClass("yui-" + N + ("-button-" + M));
      }
  }, "removeStateCSSClasses": function (M)
  {
    var N = this.get("type");
    if (I.isString(M))
      {
        this.removeClass(this.CSS_CLASS_NAME + ("-" + M));
        this.removeClass("yui-" + N + ("-button-" + M));
      }
  }, "createHiddenFields": function ()
  {
    ...
    var R = this.getForm();
    if (R && !this.get("disabled"))
      {
        N = this.get("type");
        P = N == "checkbox" || N == "radio";
        if (P || E == this)
          {
            U = F(P ? N : "hidden", this.get("name"), this.get("value"), this.get("checked"));
            if (U)
              {
                if (P)
                  {
        ...
        S = this._menu;
        if (J && S && S instanceof J)
          {
            M = S.srcElement;
            T = this.get("selectedMenuItem");
            if (T)
              {
                if (M && M.nodeName.toUpperCase() == "SELECT")
                  {
                    R.appendChild(M);
                    M.selectedIndex = T.index;
                  }
                else
                  {
                    Q = T.value === null || T.value === "" ? T.cfg.getProperty("text") : T.value;
                    O = this.get("name");
                    if (Q && O)
                      {
                        M = F("hidden", O + "_options", Q);
                        R.appendChild(M);
                      }
                  }
              }
        ...
        if (U && M)
          {
            this._hiddenFields = [U,M];
          }
        else
          {
            if (!U && M)
              {
                this._hiddenFields = M;
              }
            else
              {
                if (U && !M)
                  {
                    this._hiddenFields = U;
                  }
              }
          }
  ...
  {
    var M;
    var P = this.getForm(), O = this.get("srcelement"), N = false;
    if (P)
      {
        if (this.get("type") == "submit" || O && O.type == "submit")
          {
            E = this;
          }
        if (YAHOO.env.ua.ie)
          {
            N = P.fireEvent("onsubmit");
          }
        else
          {
            M = document.createEvent("HTMLEvents");
            M.initEvent("submit", true, true);
            N = P.dispatchEvent(M);
          }
        if ((YAHOO.env.ua.ie || YAHOO.env.ua.webkit) && N)
          {
            P.submit();
          }
      }
  ...
  {
    var R, P;
    var O = T.type == "link" ? "a" : "button", Q = T.srcelement, S = M.getElementsByTagName(O)[0];
    if (!S)
      {
        R = M.getElementsByTagName("input")[0];
        if (R)
          {
    ...
    this._button = S;
    YAHOO.widget.Button.superclass.init.call(this, M, T);
    D[this.get("id")] = this;
    this.addClass(this.CSS_CLASS_NAME);
    this.addClass("yui-" + this.get("type") + "-button");
    L.on(this._button, "focus", this._onFocus, null, this);
    this.on("mouseover", this._onMouseOver);
    this.on("click", this._onClick);
    this.on("appendTo", this._onAppendTo);
    var V = this.get("container"), N = this.get("element"), U = G.inDocument(N);
    if (V)
      {
        if (Q && Q != N)
          {
        ...
        if (!U && Q && Q != N)
          {
            ...
            if (P)
              {
                this.fireEvent("beforeAppendTo", { "type": "beforeAppendTo", "target": P });
                P.replaceChild(N, Q);
  ...
  {
    var M = N || { };
    YAHOO.widget.Button.superclass.initAttributes.call(this, M);
    ...
    this.setAttributeConfig("focusmenu", { "value": M.focusmenu === false ? false : true, "validator": I.isBoolean });
  }, "focus": function ()
  {
  ...
  {
    var Q;
    var O = this.get("element"), N = O.parentNode, M = this._menu;
    if (M)
      {
        if (K && K.find(M))
          {
    ...
    if (P)
      {
        L.removeListener(P, "reset", this._onFormReset);
        L.removeListener(P, "submit", this.createHiddenFields);
      }
    this.unsubscribeAll();
    if (N)
      {
    ...
    Q = G.getElementsByClassName(this.CSS_CLASS_NAME, this.NODE_NAME, P);
    if (I.isArray(Q) && Q.length === 0)
      {
        L.removeListener(P, "keypress", YAHOO.widget.Button.onFormKeyPress);
      }
  }, "fireEvent": function (N, M)
  {
    var O = arguments[0];
    if (this.DOM_EVENTS[O] && this.get("disabled"))
      {
        return;
      }
  ...
  YAHOO.widget.Button.onFormKeyPress = function (Q)
  {
    var U, V, N, W;
    var O = L.getTarget(Q), R = L.getCharCode(Q), P = O.nodeName && O.nodeName.toUpperCase(), M = O.type, S = false;
    function T(Z)
    {
      var Y, X;
      switch (Z.nodeName.toUpperCase()) {
        case "INPUT":
        case "BUTTON":
          if (Z.type == "submit" && !Z.disabled)
            {
              if (!S && !N)
                {
                  N = Z;
              ...
              if (U)
                {
                  ...
                  if (!U.get("disabled"))
                    {
                      X = U.get("srcelement");
                      if (!V && (U.get("type") == "submit" || X && X.type == "submit"))
                        {
                          V = U;
                        }
                    }
    ...
    if (R == 13 && (P == "INPUT" && (M == "text" || M == "password" || M == "checkbox" || M == "radio" || M == "file") || P == "SELECT"))
      {
        G.getElementsBy(T, "*", this);
        if (N)
          {
            N.focus();
          }
  ...
  YAHOO.widget.Button.addHiddenFieldsToForm = function (M)
  {
    var Q, N, O;
    var R = G.getElementsByClassName(YAHOO.widget.Button.prototype.CSS_CLASS_NAME, "*", M), P = R.length;
    if (P > 0)
      {
        for (O = 0; O < P; ++O)
          {
            ...
            if (N)
              {
                ...
                if (Q)
                  {
                    Q.createHiddenFields();
                  }
              }
          }
      }
  };
  YAHOO.widget.Button.getButton = function (M)
  {
    var N = D[M];
    if (N)
      {
...
(function ()
{
  ...
  {
    var F;
    var I = this._buttons, G = I.length;
    for (F = 0; F < G; ++F)
      {
  ...
  {
    var G;
    var I = this._buttons.length, H = this.get("element"), F = H.parentNode;
    if (I > 0)
      {
...
YAHOO.register("button", YAHOO.widget.Button, { "version": "2.5.0", "build": "897" });
(function ()
{
...
(function ()
{
  var B = YAHOO.util.Dom, A = YAHOO.util.Event, D = YAHOO.lang;
  YAHOO.widget.Toolbar = function (G, F)
  {
    if (D.isObject(arguments[0]) && !B.get(G).nodeType)
      {
        F = G;
      }
    var I = F || { };
    var H = { "element": null, "attributes": I };
    if (D.isString(G) && B.get(G))
      {
        H.element = B.get(G);
      }
    else
      {
        if (D.isObject(G) && B.get(G) && B.get(G).nodeType)
          {
            H.element = B.get(G);
          }
      }
  ...
  }, "initAttributes": function (E)
  {
    ...
    this.setAttributeConfig("grouplabels", { "value": E.grouplabels === false ? false : true, "method": function (F)
    {
      if (F)
        {
          B.removeClass(this.get("cont"), this.CLASS_PREFIX + "-nogrouplabels");
        }
      else
        {
          B.addClass(this.get("cont"), this.CLASS_PREFIX + "-nogrouplabels");
        }
    } });
      ...
      if (G)
        {
          ...
          B.addClass(this._titlebar, this.CLASS_PREFIX + "-titlebar");
          if (D.isString(G))
            {
          ...
          if (H)
            {
              ...
              B.addClass(G, "collapse");
              this._titlebar.appendChild(G);
              A.addListener(G, "click", function ()
              {
                if (B.hasClass(this.get("cont").parentNode, "yui-toolbar-container-collapsed"))
                  {
                    this.collapse(false);
                  }
                else
            ...
            {
              G = B.getElementsByClassName("collapse", "span", this._titlebar);
              if (G[0])
                {
                  if (B.hasClass(this.get("cont").parentNode, "yui-toolbar-container-collapsed"))
                    {
          ...
          if (!this._dragHandle)
            {
              ...
              B.addClass(this._dragHandle, this.CLASS_DRAGHANDLE);
          ...
          if (F)
            {
              ...
              B.addClass(this._titlebar, "draggable");
            }
          else
            {
              B.removeClass(this._titlebar, "draggable");
              if (this.dd)
                {
                  this.dd.unreg();
                  this.dd = null;
  ...
  }, "addButtonGroup": function (I)
  {
    ...
    B.addClass(J, this.CLASS_PREFIX + "-group");
    B.addClass(J, this.CLASS_PREFIX + "-group-" + I.group);
    if (I.label)
      {
    ...
    if (N)
    ...
    else
      {
        ...
        if (!T.buttonType)
          {
            T.buttonType = "rich";
            T.checkValue = function (Y)
            {
              var X = this.getMenu().getItems();
              if (X.length === 0)
                {
                  this.getMenu()._onBeforeShow();
                  X = this.getMenu().getItems();
                }
              for (var W = 0; W < X.length; ++W)
                {
                  X[W].cfg.setProperty("checked", false);
                  if (X[W].value == Y)
                    {
                      X[W].cfg.setProperty("checked", true);
                    }
                }
            };
          }
                  ...
                  if (W)
                    {
                      ...
                      if (B.getStyle(X, "visibility") == "hidden")
                        {
                          T.getMenu().show();
                        }
                      else
            ...
            else
              {
                if (J.type != "menu" && J.type != "select")
                ...
                else
                  {
                    ...
                    var K = this;
                    if (T.getMenu().mouseDownEvent)
                      {
                        T.getMenu().mouseDownEvent.subscribe(function (Y, X)
                        {
                          var W = X[1];
                          YAHOO.util.Event.stopEvent(X[0]);
                          T._onMenuClick(X[0], T);
                          if (!J.menucmd)
                            {
                              J.menucmd = J.value;
                            }
                          J.value = W.value ? W.value : W._oText.nodeValue;
                          K._buttonClick.call(K, X[1], J);
                          T._hideMenu();
                          return false;
                        });
                        T.getMenu().clickEvent.subscribe(function (X, W)
                        {
                          YAHOO.util.Event.stopEvent(W[0]);
                        });
                        T.getMenu().mouseUpEvent.subscribe(function (X, W)
                        {
                          YAHOO.util.Event.stopEvent(W[0]);
                        });
                      }
        ...
        if (this.browser.ie)
          {
            T.DOM_EVENTS.focusin = true;
            T.DOM_EVENTS.focusout = true;
            T.on("focusin", function (W)
            {
              YAHOO.util.Event.stopEvent(W);
            }, J, this);
            T.on("focusout", function (W)
            {
              YAHOO.util.Event.stopEvent(W);
            }, J, this);
            T.on("click", function (W)
    ...
    if (!this._sep)
      {
        ...
        B.addClass(this._sep, this.CLASS_SEPARATOR);
        this._sep.innerHTML = "|";
      }
    var G = this._sep.cloneNode(true);
    this._sepCount++;
    B.addClass(G, this.CLASS_SEPARATOR + "-" + this._sepCount);
    if (H)
      {
        var I = null;
        if (H.get)
  ...
  }, "_createColorPicker": function (H)
  {
    if (B.get(H + "_colors"))
      {
        B.get(H + "_colors").parentNode.removeChild(B.get(H + "_colors"));
      }
    var E = document.createElement("div");
  ...
  }, "_makeColorButton": function (E)
  {
    ...
    B.addClass(E.menu.element, "yui-button-menu");
    B.addClass(E.menu.element, "yui-color-button-menu");
  ...
  }, "getButtonById": function (G)
  {
    ...
    for (var F = 0; F < E; ++F)
      {
        if (this._buttonList[F].get("id") == G)
          {
    ...
    if (I)
      {
        if (D.isString(I))
          {
            F = this.getButtonById(I);
          }
        if (D.isNumber(I))
          {
            F = this.getButtonByIndex(I);
          }
        if (!(F instanceof YAHOO.widget.ToolbarButton) && !(F instanceof YAHOO.widget.ToolbarButtonAdvanced))
          {
            F = this.getButtonByValue(I);
          }
        if (F instanceof YAHOO.widget.ToolbarButton || F instanceof YAHOO.widget.ToolbarButtonAdvanced)
          {
  ...
  }, "destroyButton": function (I)
  {
    ...
    if (D.isString(I))
      {
        G = this.getButtonById(I);
      }
    if (D.isNumber(I))
      {
        G = this.getButtonByIndex(I);
      }
    if (!(G instanceof YAHOO.widget.ToolbarButton) && !(G instanceof YAHOO.widget.ToolbarButtonAdvanced))
      {
        G = this.getButtonByValue(I);
      }
    if (G instanceof YAHOO.widget.ToolbarButton || G instanceof YAHOO.widget.ToolbarButtonAdvanced)
      {
        var H = G.get("id");
        ...
        for (var F = 0; F < E; ++F)
          {
            if (this._buttonList[F].get("id") == H)
              {
  ...
  {
    var E = B.getElementsByClassName("collapse", "span", this._titlebar);
    if (F === false)
      {
        B.removeClass(this.get("cont").parentNode, "yui-toolbar-container-collapsed");
        if (E[0])
          {
            B.removeClass(E[0], "collapsed");
          }
    ...
    else
      {
        if (E[0])
          {
            B.addClass(E[0], "collapsed");
          }
        B.addClass(this.get("cont").parentNode, "yui-toolbar-container-collapsed");
...
(function ()
{
  ...
  YAHOO.widget.SimpleEditor = function (I, N)
  {
    var H = { };
    if (D.isObject(I) && !I.tagName && !N)
      {
        D.augmentObject(H, I);
    ...
    var J = { "element": null, "attributes": H }, G = null;
    if (D.isString(I))
      {
        G = I;
      }
    ...
    J.attributes.element_cont = new YAHOO.util.Element(K, { "id": G + "_container" });
    var F = document.createElement("div");
    C.addClass(F, "first-child");
    J.attributes.element_cont.appendChild(F);
    if (!J.attributes.toolbar_cont)
      {
        J.attributes.toolbar_cont = document.createElement("DIV");
        J.attributes.toolbar_cont.id = G + "_toolbar";
        F.appendChild(J.attributes.toolbar_cont);
      }
    var M = document.createElement("DIV");
    F.appendChild(M);
    J.attributes.editor_wrapper = M;
    YAHOO.widget.SimpleEditor.superclass.constructor.call(this, J.element, J.attributes);
  };
  function E(F)
  {
    return F.replace(/ /g, "-").toLowerCase();
  }
  YAHOO.extend(YAHOO.widget.SimpleEditor, YAHOO.util.Element, { "_docType": "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01//EN\" \"http://www.w3.org/TR/html4/strict.dtd\">", "editorDirty": null, "_defaultCSS": "html { height: 95%; } body { padding: 7px; background-color: #fff; font:13px/1.22 arial,helvetica,clean,sans-serif;*font-size:small;*font:x-small; } a { color: blue; text-decoration: underline; cursor: pointer; } .warning-localfile { border-bottom: 1px dashed red !important; } .yui-busy { cursor: wait !important; } img.selected { border: 2px dotted #808080; } img { cursor: pointer !important; border: none; }", "_defaultToolbar": null, "_lastButton": null, "_baseHREF": function ()
  {
  ...
  }(), "_lastImage": null, "_blankImageLoaded": null, "_fixNodesTimer": null, "_nodeChangeTimer": null, "_lastNodeChangeEvent": null, "_lastNodeChange": 0, "_rendered": null, "DOMReady": null, "_selection": null, "_mask": null, "_showingHiddenElements": null, "currentWindow": null, "currentEvent": null, "operaEvent": null, "currentFont": null, "currentElement": null, "dompath": null, "beforeElement": null, "afterElement": null, "invalidHTML": { "form": true, "input": true, "button": true, "select": true, "link": true, "html": true, "body": true, "iframe": true, "script": true, "style": true, "textarea": true }, "toolbar": null, "_contentTimer": null, "_contentTimerCounter": 0, "_disabled": ["createlink","fontname","fontsize","forecolor","backcolor"], "_alwaysDisabled": { }, "_alwaysEnabled": { }, "_semantic": { "bold": true, "italic": true, "underline": true }, "_tag2cmd": { "b": "bold", "strong": "bold", "i": "italic", "em": "italic", "u": "underline", "sup": "superscript", "sub": "subscript", "img": "insertimage", "a": "createlink", "ul": "insertunorderedlist", "ol": "insertorderedlist" }, "_createIframe": function ()
  {
    ...
    var F = new YAHOO.util.Element(J);
    return F;
  }, "_isElement": function (G, F)
  {
    if (G && G.tagName && G.tagName.toLowerCase() == F)
      {
        return true;
      }
    if (G && G.getAttribute && G.getAttribute("tag") == F)
      {
        return true;
      }
    return false;
  }, "_hasParent": function (G, F)
  {
    if (!G || !G.parentNode)
      {
        return false;
                      ...
                      if (this.get("iframe").get("element").contentWindow)
                        {
                          if (this.get("iframe").get("element").contentWindow.document)
                            {
                              F = this.get("iframe").get("element").contentWindow.document;
                              return F;
                            }
                        }
                    }
    ...
    if (this._getDoc() && this._getWindow())
      {
        if (this._getDoc().selection)
          {
            F = this._getDoc().selection;
          }
        else
          {
            F = this._getWindow().getSelection();
          }
        if (this.browser.webkit)
          {
            if (F.baseNode)
            ...
            else
              {
                if (this._selection !== null)
                  {
                    F = this._getWindow().getSelection();
                    F.setBaseAndExtent(this._selection.baseNode, this._selection.baseOffset, this._selection.extentNode, this._selection.extentOffset);
    ...
    if (this.browser.ie)
      {
        try 
        {
          F = this._getDoc().body.createTextRange();
    ...
    else
      {
        if (this.browser.webkit)
          {
            H.setBaseAndExtent(G, 0, G, G.innerText.length);
          }
        else
          {
  ...
  }, "_getRange": function ()
  {
    ...
    if (this.browser.webkit && !F.getRangeAt)
      {
    ...
    if (F.rangeCount > 0)
      {
        return F.getRangeAt(0);
      }
    return null;
  }, "_setDesignMode": function (F)
  {
    try 
    {
      var H = true;
      if (this.browser.ie && F.toLowerCase() == "off")
        {
          H = false;
        }
      if (H)
        {
          this._getDoc().designMode = F;
        }
    }
    catch (G)
    {
  ...
  {
    var G = this._getDoc().designMode.toLowerCase(), F = "on";
    if (G == "on")
      {
        F = "off";
      }
  ...
  }, "_initEditor": function ()
  {
    ...
    A.on(this._getDoc(), "mouseup", this._handleMouseUp, this, true);
    A.on(this._getDoc(), "mousedown", this._handleMouseDown, this, true);
    A.on(this._getDoc(), "click", this._handleClick, this, true);
    A.on(this._getDoc(), "dblclick", this._handleDoubleClick, this, true);
    A.on(this._getDoc(), "keypress", this._handleKeyPress, this, true);
    A.on(this._getDoc(), "keyup", this._handleKeyUp, this, true);
    A.on(this._getDoc(), "keydown", this._handleKeyDown, this, true);
    if (!this.get("disabled"))
      {
  ...
  }, "_checkLoaded": function ()
  {
    ...
    if (H === true)
    ...
    else
      {
        ...
        {
          F._checkLoaded.call(F);
        }, 20);
      }
  }, "_setInitialContent": function ()
  {
    var G = D.substitute(this.get("html"), { "TITLE": this.STR_TITLE, "CONTENT": this._cleanIncomingHTML(this.get("element").value), "CSS": this.get("css"), "HIDDEN_CSS": this.get("hiddencss") ? this.get("hiddencss") : "/* No Hidden CSS */", "EXTRA_CSS": this.get("extracss") ? this.get("extracss") : "/* No Extra CSS */" }), F = true;
  ...
  }, "_setEditorStyle": function (G)
  {
    try 
    {
      this._getDoc().execCommand("useCSS", false, !G);
    }
    catch (F)
    {
    }
  ...
  {
    var I = this._getDoc(), F = null, G = null, J = null;
    if (this.browser.ie)
      {
        ...
        if (F)
          {
            J = F.item ? F.item(0) : F.parentElement();
            if (J == I.body)
              {
                J = null;
              }
        ...
        if (this.currentEvent !== null && this.currentEvent.keyCode === 0)
          {
            J = A.getTarget(this.currentEvent);
          }
      }
    else
      {
        ...
        if (!this._hasSelection())
          {
            if (G.anchorNode && G.anchorNode.nodeType == 3)
              {
                if (G.anchorNode.parentNode)
                  {
                    J = G.anchorNode.parentNode;
                  }
                if (G.anchorNode.nextSibling != G.focusNode.nextSibling)
                  {
                    J = G.anchorNode.nextSibling;
                  }
              }
            if (this._isElement(J, "br"))
              {
                J = null;
              }
            if (!J)
                        ...
                        if (F.startOffset - F.endOffset < 2)
                          {
                            if (F.startContainer.hasChildNodes())
                              {
                                J = F.startContainer.childNodes[F.startOffset];
                              }
                          }
                      }
                  }
    ...
    if (this.currentEvent !== null)
    ...
    else
      {
        if (this.currentElement && this.currentElement[0] && !this.browser.ie)
          {
            J = this.currentElement[0];
          }
      }
    if (this.browser.opera || this.browser.webkit)
      {
        if (this.currentEvent && !J)
          {
            J = YAHOO.util.Event.getTarget(this.currentEvent);
          }
      }
    if (!J || !J.tagName)
      {
  ...
  }, "_getDomPath": function (F)
  {
    ...
    while (F !== null)
      {
        if (F.ownerDocument != this._getDoc())
          {
            F = null;
            break;
          }
        if (F.nodeName && F.nodeType && F.nodeType == 1)
          {
            G[G.length] = F;
          }
        if (this._isElement(F, "body"))
          {
            break;
          }
        F = F.parentNode;
    ...
    if (G.length === 0)
      {
        if (this._getDoc() && this._getDoc().body)
          {
            G[0] = this._getDoc().body;
          }
      }
    return G.reverse();
  }, "_writeDomPath": function ()
  {
    var L = this._getDomPath(), J = [], H = "", M = "";
    for (var F = 0; F < L.length; ++F)
      {
        var N = L[F].tagName.toLowerCase();
        if (N == "ol" && L[F].type)
          {
            N += ":" + L[F].type;
          }
        if (C.hasClass(L[F], "yui-tag"))
          {
            N = L[F].getAttribute("tag");
          }
        ...
        if (!C.hasClass(L[F], "yui-non"))
          {
            if (C.hasClass(L[F], "yui-tag"))
              ...
              {
                H = L[F].className !== "" ? "." + L[F].className.replace(/ /g, ".") : "";
                if (H.indexOf("yui") != -1 || H.toLowerCase().indexOf("apple-style-span") != -1)
                  {
                    H = "";
                  }
                M = N + (L[F].id ? "#" + L[F].id : "") + H;
            ...
            switch (N) {
              case "a":
                if (L[F].getAttribute("href", 2))
                  {
                    M += ":" + L[F].getAttribute("href", 2).replace("mailto:", "").replace("http://", "").replace("https://", "");
                  }
                break;
              case "img":
                var G = L[F].height;
                ...
                if (L[F].style.height)
                  {
                    G = parseInt(L[F].style.height, 10);
                  }
                if (L[F].style.width)
                  {
                    K = parseInt(L[F].style.width, 10);
                  }
                M += "(" + G + "x" + K + ")";
                break;
            }
            if (M.length > 10)
              {
                M = "<span title=\"" + M + "\">" + M.substring(0, 10) + "...</span>";
              }
            else
              {
                M = "<span title=\"" + M + "\">" + M + "</span>";
              }
            J[J.length] = M;
          }
      }
    var I = J.join(" " + this.SEP_DOMPATH + " ");
    if (this.dompath.innerHTML != I)
      {
        this.dompath.innerHTML = I;
      }
  }, "_fixNodes": function ()
  {
    var K = this._getDoc(), I = [];
    for (var F in this.invalidHTML)
      {
        if (YAHOO.lang.hasOwnProperty(this.invalidHTML, F))
          {
            if (F.toLowerCase() != "span")
              {
                var G = K.body.getElementsByTagName(F);
                if (G.length)
                  {
    ...
    for (var J = 0; J < I.length; ++J)
      {
        if (I[J].parentNode)
          {
            if (D.isObject(this.invalidHTML[I[J].tagName.toLowerCase()]) && this.invalidHTML[I[J].tagName.toLowerCase()].keepContents)
              {
                this._swapEl(I[J], "span", function (M)
                {
                  M.className = "yui-non";
                });
              }
  ...
  }, "_isNonEditable": function (H)
  {
    if (this.get("allowNoEdit"))
      {
        var G = A.getTarget(H);
        if (this._isElement(G, "html"))
          {
            G = null;
          }
        var J = this._getDomPath(G);
        for (var F = J.length - 1; F > -1; --F)
          {
            if (C.hasClass(J[F], this.CLASS_NOEDIT))
              {
                try 
                {
  ...
  }, "_handleClick": function (G)
  {
    ...
    if (this.browser.webkit)
      {
        var F = A.getTarget(G);
        if (this._isElement(F, "a") || this._isElement(F.parentNode, "a"))
          {
            A.stopEvent(G);
            this.nodeChange();
          }
  ...
  }, "_handleKeyUp": function (G)
  {
    ...
    switch (G.keyCode) {
      ...
      case 8:
      case 87:
        if (G.keyCode == 87 && this.currentWindow && G.shiftKey && G.ctrlKey)
          {
            this.closeWindow();
          }
        else
  ...
  }, "_handleKeyPress": function (F)
  {
    if (this.get("allowNoEdit"))
      {
        if (F && F.keyCode && (F.keyCode == 46 || F.keyCode == 63272))
          {
            A.stopEvent(F);
          }
      }
    ...
    if (this.browser.webkit)
      {
        if (!this.browser.webkit3)
          {
            if (F.keyCode && F.keyCode == 122 && F.metaKey)
              {
    ...
    this.fireEvent("editorKeyPress", { "type": "editorKeyPress", "target": this, "ev": F });
  }, "_listFix": function (L)
  {
    var O = null, J = null, F = false, H = null;
    if (this.browser.webkit)
      {
        if (L.keyCode && L.keyCode == 13)
          {
            if (this._hasParent(this._getSelectedElement(), "li"))
              {
                var I = this._hasParent(this._getSelectedElement(), "li");
                var N = this._getDoc().createElement("li");
                N.innerHTML = "<span class=\"yui-non\">&nbsp;</span>&nbsp;";
                if (I.nextSibling)
                  {
    ...
    if (L.keyCode && (!this.browser.webkit3 && L.keyCode == 25 || (this.browser.webkit3 || !this.browser.webkit) && (L.keyCode == 9 && L.shiftKey)))
      {
        O = this._getSelectedElement();
        if (this._hasParent(O, "li"))
          {
            O = this._hasParent(O, "li");
            if (this._hasParent(O, "ul") || this._hasParent(O, "ol"))
              {
                J = this._hasParent(O, "ul");
                if (!J)
                  {
                    J = this._hasParent(O, "ol");
                  }
                if (this._isElement(J.previousSibling, "li"))
                  {
                    J.removeChild(O);
                    J.parentNode.insertBefore(O, J.nextSibling);
                    if (this.browser.ie)
                      {
                        H = this._getDoc().body.createTextRange();
                    ...
                    if (this.browser.webkit)
                      {
                        if (!this.browser.webkit3)
                          {
                            J.style.display = "list-item";
                            J.parentNode.style.display = "list-item";
                            setTimeout(function ()
                            {
    ...
    if (L.keyCode && (L.keyCode == 9 && !L.shiftKey))
      {
        var G = this._getSelectedElement();
        if (this._hasParent(G, "li"))
          {
            F = this._hasParent(G, "li").innerHTML;
          }
        if (this.browser.webkit)
          {
        ...
        if (this._hasParent(O, "li"))
          {
            J = this._hasParent(O, "li");
            var K = this._getDoc().createElement(J.parentNode.tagName.toLowerCase());
            if (this.browser.webkit)
              {
                var M = C.getElementsByClassName("Apple-tab-span", "span", J);
                if (M[0])
                  {
                    J.removeChild(M[0]);
                    J.innerHTML = D.trim(J.innerHTML);
                    if (F)
                      {
                        J.innerHTML = "<span class=\"yui-non\">" + F + "</span>&nbsp;";
                      }
                    else
                      {
                        J.innerHTML = "<span class=\"yui-non\">&nbsp;</span>&nbsp;";
                      }
                  }
              }
            else
            ...
            if (this.browser.webkit)
              {
                this._getSelection().setBaseAndExtent(J.firstChild, 1, J.firstChild, J.firstChild.innerText.length);
                if (!this.browser.webkit3)
                  {
                    J.parentNode.parentNode.style.display = "list-item";
                    setTimeout(function ()
                    {
                      J.parentNode.parentNode.style.display = "block";
                    }, 1);
                  }
              }
            else
  ...
  }, "_handleKeyDown": function (J)
  {
    if (this._isNonEditable(J))
      {
        return false;
      }
    this._setCurrentEvent(J);
    if (this.currentWindow)
      {
        this.closeWindow();
      }
    ...
    var I = false, K = null, H = false;
    if (J.shiftKey && J.ctrlKey)
      {
        I = true;
      }
    switch (J.keyCode) {
      ...
      case 76:
        ...
        break;
      case 65:
        if (J.metaKey && this.browser.webkit)
          {
            A.stopEvent(J);
    ...
    if (I && K)
      {
        this.execCommand(K, null);
        A.stopEvent(J);
        this.nodeChange();
      }
    this.fireEvent("editorKeyDown", { "type": "editorKeyDown", "target": this, "ev": J });
  }, "nodeChange": function (G)
  {
        ...
        if (this._fixNodesTimer === null)
          {
            ...
            {
              Q._fixNodes.call(Q);
              Q._fixNodesTimer = null;
            }, 0);
          }
      }
    this._lastNodeChange = N;
    if (this.currentEvent)
      {
        this._lastNodeChangeEvent = this.currentEvent.type;
      }
    var Y = this.fireEvent("beforeNodeChange", { "type": "beforeNodeChange", "target": this });
    if (Y === false)
      {
        return false;
      }
    ...
    if (!this.get("disabled"))
      {
        if (this.STOP_NODE_CHANGE)
          ...
          {
            var S = this._getSelection(), P = this._getRange(), F = this._getSelectedElement(), L = this.toolbar.getButtonByValue("fontname"), K = this.toolbar.getButtonByValue("fontsize");
            if (G !== true)
              {
                this.editorDirty = true;
              }
            ...
            this.toolbar.resetAllButtons(M);
            for (var Z = 0; Z < this._disabled.length; ++Z)
              {
                var O = this.toolbar.getButtonByValue(this._disabled[Z]);
                if (O && O.get)
                  {
                        ...
                        if (!this._hasSelection())
                          {
                            switch (this._disabled[Z]) {
                              ...
                              case "fontsize":
                                break;
                              default:
                                this.toolbar.disableButton(O);
                            }
                          }
                        else
                          {
                            if (!this._alwaysDisabled[this._disabled[Z]])
                              {
                                this.toolbar.enableButton(O);
                              }
                          }
                        if (!this._alwaysEnabled[this._disabled[Z]])
                          {
                            this.toolbar.deselectButton(O);
                          }
                      }
                  }
              }
            var R = this._getDomPath();
            var a = null, V = null;
            for (var W = 0; W < R.length; ++W)
              {
                a = R[W].tagName.toLowerCase();
                if (R[W].getAttribute("tag"))
                  {
                    a = R[W].getAttribute("tag").toLowerCase();
                  }
                V = this._tag2cmd[a];
                if (V === undefined)
                  {
                ...
                if (R[W].style.fontWeight.toLowerCase() == "bold")
                  {
                    V[V.length] = "bold";
                  }
                if (R[W].style.fontStyle.toLowerCase() == "italic")
                  {
                    V[V.length] = "italic";
                  }
                if (R[W].style.textDecoration.toLowerCase() == "underline")
                  {
                    V[V.length] = "underline";
                  }
                if (V.length > 0)
                  {
                    for (var U = 0; U < V.length; ++U)
                      {
                        this.toolbar.selectButton(V[U]);
                        this.toolbar.enableButton(V[U]);
                      }
                  }
                switch (R[W].style.textAlign.toLowerCase()) {
                  case "left":
                  case "right":
                  case "center":
                  case "justify":
                    var T = R[W].style.textAlign.toLowerCase();
                    if (R[W].style.textAlign.toLowerCase() == "justify")
                      {
                        T = "full";
                      }
                    this.toolbar.selectButton("justify" + T);
                    this.toolbar.enableButton("justify" + T);
                    break;
                }
              }
            if (L)
              {
                var X = L._configs.label._initialConfig.value;
                L.set("label", "<span class=\"yui-toolbar-fontname-" + E(X) + "\">" + X + "</span>");
                this._updateMenuChecked("fontname", X);
              }
            if (K)
              {
                K.set("label", K._configs.label._initialConfig.value);
              }
            var J = this.toolbar.getButtonByValue("heading");
            if (J)
              {
                J.set("label", J._configs.label._initialConfig.value);
  ...
  }, "_updateMenuChecked": function (F, G, I)
  {
    if (!I)
      {
        I = this.toolbar;
      }
    var H = I.getButtonByValue(F);
    H.checkValue(G);
  }, "_handleToolbarClick": function (G)
  {
    var I = "";
    var J = "";
    ...
    if (this.STOP_EXEC_COMMAND)
      ...
      {
        this.execCommand(H, I);
        if (!this.browser.webkit)
          {
            var F = this;
            setTimeout(function ()
            {
              F._focusWindow.call(F);
            }, 5);
          }
      }
    A.stopEvent(G);
  ...
  }(), "init": function (G, F)
  {
    ...
    YAHOO.widget.SimpleEditor.superclass.init.call(this, G, F);
  ...
  {
    YAHOO.widget.SimpleEditor.superclass.initAttributes.call(this, F);
    var G = this;
    ...
    this.setAttributeConfig("height", { "value": F.height || C.getStyle(G.get("element"), "height"), "method": function (H)
    {
      if (this._rendered)
        {
          if (this.get("animate"))
            {
              var I = new YAHOO.util.Anim(this.get("iframe").get("parentNode"), { "height": { "to": parseInt(H, 10) } }, 0.5);
              I.animate();
            }
          else
            {
              C.setStyle(this.get("iframe").get("parentNode"), "height", H);
            }
        }
    } });
    this.setAttributeConfig("autoHeight", { "value": F.autoHeight || false, "method": function (H)
    {
      if (H)
        {
          if (this.get("iframe"))
    ...
    this.setAttributeConfig("width", { "value": F.width || C.getStyle(this.get("element"), "width"), "method": function (H)
    {
      if (this._rendered)
        {
          if (this.get("animate"))
            {
              var I = new YAHOO.util.Anim(this.get("element_cont").get("element"), { "width": { "to": parseInt(H, 10) } }, 0.5);
              I.animate();
            }
          else
            {
              this.get("element_cont").setStyle("width", H);
            }
        }
    } });
    ...
    this.setAttributeConfig("handleSubmit", { "value": F.handleSubmit || false, "method": function (H)
    {
          ...
          if (H)
            {
              ...
              for (var K = 0; K < I.length; ++K)
                {
                  var J = I[K].getAttribute("type");
                  if (J && J.toLowerCase() == "submit")
                    {
                      A.on(I[K], "click", this._handleFormButtonClick, this, true);
            ...
            {
              A.unsubscribe(this.get("element").form, "submit", this._handleFormSubmit);
              if (this._formButtons)
                {
                  A.unsubscribe(this._formButtons, "click", this._handleFormButtonClick);
                }
            }
        }
    } });
    this.setAttributeConfig("disabled", { "value": false, "method": function (H)
    {
      if (this._rendered)
        {
          this._disableEditor(H);
        }
    } });
    this.setAttributeConfig("toolbar_cont", { "value": null, "writeOnce": true });
    this.setAttributeConfig("toolbar", { "value": F.toolbar || this._defaultToolbar, "writeOnce": true, "method": function (H)
    {
      if (!H.buttonType)
        {
          H.buttonType = this._defaultToolbar.buttonType;
        }
      this._defaultToolbar = H;
    } });
    this.setAttributeConfig("animate", { "value": F.animate ? YAHOO.util.Anim ? true : false : false, "validator": function (I)
    {
      var H = true;
      if (!YAHOO.util.Anim)
        {
    ...
    this.setAttributeConfig("panel", { "value": null, "writeOnce": true, "validator": function (I)
    {
      var H = true;
      if (!YAHOO.widget.Overlay)
        {
    ...
    this.setAttributeConfig("focusAtStart", { "value": F.focusAtStart || false, "writeOnce": true, "method": function ()
    {
      this.on("editorContentLoaded", function ()
      {
        ...
        setTimeout(function ()
        {
          H._focusWindow.call(H, true);
          H.editorDirty = false;
        }, 400);
      }, this, true);
    } });
    this.setAttributeConfig("dompath", { "value": F.dompath || false, "method": function (H)
    {
      if (H && !this.dompath)
        {
    ...
    this.setAttributeConfig("markup", { "value": F.markup || "semantic", "validator": function (H)
    {
    ...
    if (!this._blankImageLoaded)
      {
        ...
        G.style.top = "-9999px";
        G.style.left = "-9999px";
        G.className = this.CLASS_PREFIX + "-blankimage";
        document.body.appendChild(G);
        F = YAHOO.util.Dom.getStyle(G, "background-image");
      ...
      {
        F = this.get("blankimage");
      }
    return F;
  }, "_handleAutoHeight": function ()
  {
    var J = this._getDoc(), G = J.body, K = J.documentElement;
    ...
    if (H < parseInt(this.get("height"), 10))
      {
        H = parseInt(this.get("height"), 10);
      }
    if (F != H && H >= parseInt(this.get("height"), 10))
      {
        C.setStyle(this.get("editor_wrapper"), "height", H + "px");
        if (this.browser.ie)
          {
            ...
            window.setTimeout(function ()
            {
              I.get("iframe").setStyle("height", "100%");
            }, 1);
          }
      }
  }, "_formButtons": null, "_formButtonClicked": null, "_handleFormButtonClick": function (G)
  {
    var F = A.getTarget(G);
  ...
  }, "_handleFormSubmit": function (I)
  {
    ...
    window.setTimeout(function ()
    {
      YAHOO.util.Event.removeListener(H, "submit", G._handleFormSubmit);
      if (YAHOO.env.ua.ie)
        {
          H.fireEvent("onsubmit");
          if (F)
            {
              F.click();
            }
      ...
      else
        {
          if (F)
          ...
          else
            {
              ...
              J.initEvent("submit", true, true);
              H.dispatchEvent(J);
              if (YAHOO.env.ua.webkit)
                {
                  if (YAHOO.lang.isFunction(H.submit))
                    {
                      H.submit();
                    }
                }
  ...
  }, "_handleFontSize": function (H)
  {
    var F = this.toolbar.getButtonById(H.button.id);
    var G = F.get("label") + "px";
    this.execCommand("fontsize", G);
    this.STOP_EXEC_COMMAND = true;
  }, "_handleColorPicker": function (H)
  {
    var G = H.button;
    var F = "#" + H.color;
    if (G == "forecolor" || G == "backcolor")
      {
        this.execCommand(G, F);
      }
  }, "_handleAlign": function (I)
  {
    var H = null;
    for (var F = 0; F < I.button.menu.length; ++F)
      {
        if (I.button.menu[F].value == I.button.value)
          {
            H = I.button.menu[F].value;
          }
      }
  ...
  {
    var R = this._getDomPath(), M = null, I = null, N = null, G = false;
    var K = this.toolbar.getButtonByValue("fontname");
    var L = this.toolbar.getButtonByValue("fontsize");
    var F = this.toolbar.getButtonByValue("heading");
    for (var H = 0; H < R.length; ++H)
      {
        M = R[H];
        var Q = M.tagName.toLowerCase();
        if (M.getAttribute("tag"))
          {
            Q = M.getAttribute("tag");
          }
        I = M.getAttribute("face");
        if (C.getStyle(M, "font-family"))
          {
            I = C.getStyle(M, "font-family");
          }
        if (Q.substring(0, 1) == "h")
          {
            if (F)
              {
                for (var J = 0; J < F._configs.menu.value.length; ++J)
                  {
                    if (F._configs.menu.value[J].value.toLowerCase() == Q)
                      {
                        F.set("label", F._configs.menu.value[J].text);
                      }
                  }
                this._updateMenuChecked("heading", Q);
              }
          }
      }
    if (K)
      {
        for (var P = 0; P < K._configs.menu.value.length; ++P)
          {
            if (I && K._configs.menu.value[P].text.toLowerCase() == I.toLowerCase())
              {
                G = true;
                I = K._configs.menu.value[P].text;
              }
          }
        if (!G)
          {
            I = K._configs.label._initialConfig.value;
          }
        var O = "<span class=\"yui-toolbar-fontname-" + E(I) + "\">" + I + "</span>";
        if (K.get("label") != O)
          {
            K.set("label", O);
            this._updateMenuChecked("fontname", I);
          }
      }
    if (L)
      {
        N = parseInt(C.getStyle(M, "fontSize"), 10);
        if (N === null || isNaN(N))
          {
            N = L._configs.label._initialConfig.value;
          }
        L.set("label", "" + N);
      }
    if (!this._isElement(M, "body") && !this._isElement(M, "img"))
      {
  ...
  }, "_handleInsertImageClick": function ()
  {
    ...
    this.on("afterExecCommand", function ()
    {
      ...
      var G = prompt(this.STR_LINK_URL + ": ", H);
      if (G !== "" && G !== null)
        {
  ...
  {
    this.nodeChange();
  }, "_isLocalFile": function (F)
  {
    if (F !== "" && (F.indexOf("file:/") != -1 || F.indexOf(":\\") != -1))
      {
        return true;
      }
    return false;
  ...
  }, "_render": function ()
  {
    ...
    if (G instanceof B)
      ...
      {
        G.disabled = true;
        this.toolbar = new B(this.get("toolbar_cont"), G);
      }
    ...
    this.toolbar.on("fontsizeClick", function (H)
    {
      this._handleFontSize(H);
    }, this, true);
    this.toolbar.on("colorPickerClicked", function (H)
    {
      this._handleColorPicker(H);
      return false;
    }, this, true);
    this.toolbar.on("alignClick", function (H)
    {
      this._handleAlign(H);
    }, this, true);
    ...
    window.setTimeout(function ()
    {
      F._setupAfterElement.call(F);
    }, 0);
    this.fireEvent("afterRender", { "type": "afterRender", "target": this });
  }, "execCommand": function (H, G)
  {
    var K = this.fireEvent("beforeExecCommand", { "type": "beforeExecCommand", "target": this, "args": arguments });
    if (K === false || this.STOP_EXEC_COMMAND)
      {
        this.STOP_EXEC_COMMAND = false;
        return false;
      }
    ...
    if (this.get("limitCommands"))
      {
        if (!this.toolbar.getButtonByValue(H))
          {
            F = false;
          }
      }
    this.editorDirty = true;
    if (typeof this["cmd_" + H.toLowerCase()] == "function" && F)
      {
    ...
    if (F)
      {
        try 
        {
          this._getDoc().execCommand(H, false, G);
        }
        catch (I)
        {
        }
    ...
    this.fireEvent("afterExecCommand", { "type": "afterExecCommand", "target": this });
  }, "cmd_backcolor": function (I)
  {
    var F = true, G = this._getSelectedElement(), H = "backcolor";
    if (this.browser.gecko || this.browser.opera)
      {
        this._setEditorStyle(true);
        H = "hilitecolor";
      }
    if (!this._isElement(G, "body"))
      {
        C.setStyle(G, "background-color", I);
        this._selectNode(G);
        F = false;
      }
    else
      {
        this._createCurrentElement("span", { "backgroundColor": I });
        this._selectNode(this.currentElement[0]);
        F = false;
      }
    return [F,H];
  }, "cmd_forecolor": function (H)
  {
    var F = true, G = this._getSelectedElement();
    if (!this._isElement(G, "body"))
      {
        C.setStyle(G, "color", H);
        this._selectNode(G);
        F = false;
      }
    else
      {
        this._createCurrentElement("span", { "color": H });
        this._selectNode(this.currentElement[0]);
        F = false;
      }
    return [F];
  }, "cmd_unlink": function (F)
  {
    this._swapEl(this.currentElement[0], "span", function (G)
    {
      G.className = "yui-non";
    });
    return [false];
  }, "cmd_createlink": function (H)
  {
    var G = this._getSelectedElement(), F = null;
    if (this._hasParent(G, "a"))
      {
        this.currentElement[0] = this._hasParent(G, "a");
      }
      ...
      {
        if (!this._isElement(G, "a"))
          {
            this._createCurrentElement("a");
            F = this._swapEl(this.currentElement[0], "a");
    ...
    return [false];
  }, "cmd_insertimage": function (K)
  {
    var F = true, G = null, J = "insertimage", I = this._getSelectedElement();
    if (K === "")
      {
        K = this.get("blankimage");
      }
    if (this._isElement(I, "img"))
      {
      ...
      {
        if (this._getDoc().queryCommandEnabled(J))
          {
            this._getDoc().execCommand("insertimage", false, K);
            var L = this._getDoc().getElementsByTagName("img");
            for (var H = 0; H < L.length; ++H)
              {
                if (!YAHOO.util.Dom.hasClass(L[H], "yui-img"))
                  {
                    YAHOO.util.Dom.addClass(L[H], "yui-img");
                    this.currentElement[0] = L[H];
                  }
              }
            F = false;
          }
          ...
          {
            if (I == this._getDoc().body)
              {
                G = this._getDoc().createElement("img");
                G.setAttribute("src", K);
                YAHOO.util.Dom.addClass(G, "yui-img");
            ...
            else
              {
                ...
                G.setAttribute("src", K);
                YAHOO.util.Dom.addClass(G, "yui-img");
                this.currentElement[0].parentNode.replaceChild(G, this.currentElement[0]);
              }
            this.currentElement[0] = G;
            F = false;
          }
    ...
    return [F];
  }, "cmd_inserthtml": function (I)
  {
    var F = true, H = "inserthtml", G = null, J = null;
    if (this.browser.webkit && !this._getDoc().queryCommandEnabled(H))
      {
        this._createCurrentElement("img");
        G = this._getDoc().createElement("span");
        G.innerHTML = I;
        this.currentElement[0].parentNode.replaceChild(G, this.currentElement[0]);
        F = false;
      }
  ...
  {
    var Q = true, T = null, M = 0, G = null, P = "", U = this._getSelectedElement(), R = "insertorderedlist";
    if (W == "ul")
      {
        R = "insertunorderedlist";
      }
    if (this.browser.webkit && !this._getDoc().queryCommandEnabled(R))
      {
        if (this._isElement(U, "li") && this._isElement(U.parentNode, W))
          {
            G = U.parentNode;
            T = this._getDoc().createElement("span");
            YAHOO.util.Dom.addClass(T, "yui-non");
            P = "";
            var F = G.getElementsByTagName("li");
            for (M = 0; M < F.length; ++M)
              {
                P += "<div>" + F[M].innerHTML + "</div>";
              }
            T.innerHTML = P;
            this.currentElement[0] = G;
          ...
          {
            this._createCurrentElement(W.toLowerCase());
            T = this._getDoc().createElement(W);
    ...
    else
      {
        ...
        if (this._isElement(G, "li") && this._isElement(G.parentNode, W) || this.browser.ie && this._isElement(this._getRange().parentElement, "li") || this.browser.ie && this._isElement(G, "ul") || this.browser.ie && this._isElement(G, "ol"))
          {
            if (this.browser.ie)
              {
                ...
                var V = this._getDoc().createElement("span");
                V.innerHTML = P;
                G.parentNode.parentNode.replaceChild(V, G.parentNode);
              }
            else
              {
                this.nodeChange();
        ...
        if (this.browser.opera)
          {
            ...
            window.setTimeout(function ()
            {
              var X = O._getDoc().getElementsByTagName("li");
              for (var Y = 0; Y < X.length; ++Y)
                {
                  if (X[Y].innerHTML.toLowerCase() == "<br>")
                    {
                      X[Y].parentNode.parentNode.removeChild(X[Y].parentNode);
                    }
                }
            }, 30);
          }
        if (this.browser.ie && Q)
          {
            var K = "";
            if (this._getRange().html)
            ...
            else
              {
                ...
                if (L.length > 1)
                  ...
                  {
                    K = "<li>" + this._getRange().text + "</li>";
                  }
              }
            this._getRange().pasteHTML("<" + W + ">" + K + "</" + W + ">");
            Q = false;
          }
      }
    return Q;
  }, "cmd_insertorderedlist": function (F)
  {
    return [this.cmd_list("ol")];
  }, "cmd_insertunorderedlist": function (F)
  {
  ...
  }, "cmd_fontname": function (H)
  {
    var F = true, G = this._getSelectedElement();
    this.currentFont = H;
    if (G && G.tagName && !this._hasSelection())
      {
        YAHOO.util.Dom.setStyle(G, "font-family", H);
        F = false;
      }
    return [F];
  }, "cmd_fontsize": function (G)
  {
    if (this.currentElement && this.currentElement.length > 0 && !this._hasSelection())
    ...
    else
      {
        if (!this._isElement(this._getSelectedElement(), "body"))
          {
            var F = this._getSelectedElement();
            YAHOO.util.Dom.setStyle(F, "fontSize", G);
            this._selectNode(F);
          }
        else
          {
            this._createCurrentElement("span", { "fontSize": G });
            this._selectNode(this.currentElement[0]);
          }
      }
    return [false];
  }, "_swapEl": function (G, F, I)
  {
    var H = this._getDoc().createElement(F);
    ...
    return H;
  }, "_createCurrentElement": function (H, U)
  {
    H = H ? H : "a";
    ...
    var X = function ()
    {
      ...
      switch (H) {
        ...
        default:
          ...
          YAHOO.util.Dom.addClass(f, "yui-tag-" + H);
          YAHOO.util.Dom.addClass(f, "yui-tag");
          f.setAttribute("tag", H);
          for (var e in U)
            {
              if (YAHOO.util.Lang.hasOwnProperty(U, e))
                {
                  f.style[e] = U[e];
                }
            }
    ...
    if (!this._hasSelection())
      {
        if (this._getDoc().queryCommandEnabled("insertimage"))
          {
            ...
            var W = this._getDoc().getElementsByTagName("img");
            for (var Z = 0; Z < W.length; ++Z)
              {
                if (W[Z].getAttribute("src", 2) == "yui-tmp-img")
                  {
                    G = X();
                    W[Z].parentNode.replaceChild(G, W[Z]);
        ...
        else
          {
            if (this.currentEvent)
              {
                b = YAHOO.util.Event.getTarget(this.currentEvent);
              }
            else
              {
                b = this._getDoc().body;
        ...
        if (b)
          {
            G = X();
            if (this._isElement(b, "body") || this._isElement(b, "html"))
              {
                if (this._isElement(b, "html"))
                  {
                    b = this._getDoc().body;
                  }
                b.appendChild(G);
            ...
            if (this.browser.webkit)
              {
                this._getSelection().setBaseAndExtent(G, 0, G, 0);
                if (this.browser.webkit3)
                  {
                    this._getSelection().collapseToStart();
                  }
    ...
    else
      {
        ...
        var J = this._getDoc().getElementsByTagName(this._getSelectedElement().parentNode.tagName);
        ...
        for (var a = 0; a < F.length; ++a)
          {
            if (YAHOO.util.Dom.getStyle(F[a], "font-family") == "yui-tmp" || F[a].face && F[a].face == "yui-tmp")
              {
                G = X();
                G.innerHTML = F[a].innerHTML;
                if (this._isElement(F[a], "ol") || this._isElement(F[a], "ul"))
                  {
                    var Y = F[a].getElementsByTagName("li")[0];
                    F[a].style.fontFamily = "inherit";
                ...
                else
                  {
                    if (this._isElement(F[a], "li"))
                    ...
                    else
                      {
                        if (F[a].parentNode)
                          {
                            F[a].parentNode.replaceChild(G, F[a]);
                            this.currentElement[this.currentElement.length] = G;
                            this.currentEvent = null;
                            if (this.browser.webkit)
                              {
                            ...
                            if (this.browser.ie && U && U.fontSize)
                              {
        ...
        var c = this.currentElement.length;
        for (var d = 0; d < c; ++d)
          {
            if (d + 1 != c)
              {
                if (this.currentElement[d] && this.currentElement[d].nextSibling)
                  {
                    if (this._isElement(this.currentElement[d], "br"))
                      {
                        this.currentElement[this.currentElement.length] = this.currentElement[d].nextSibling;
                      }
                  }
              }
          }
  ...
  {
    F = this._cleanIncomingHTML(F);
    this._getDoc().body.innerHTML = F;
    this.nodeChange();
  }, "getEditorHTML": function ()
  {
    var F = this._getDoc().body;
    if (F === null)
      {
        return null;
      }
  ...
  }, "_cleanIncomingHTML": function (F)
  {
    F = F.replace(/<strong([^>]*)>/gi, "<b$1>");
    F = F.replace(/<\/strong>/gi, "</b>");
    F = F.replace(/<embed([^>]*)>/gi, "<YUI_EMBED$1>");
    F = F.replace(/<\/embed>/gi, "</YUI_EMBED>");
    F = F.replace(/<em([^>]*)>/gi, "<i$1>");
    F = F.replace(/<\/em>/gi, "</i>");
    F = F.replace(/<YUI_EMBED([^>]*)>/gi, "<embed$1>");
    F = F.replace(/<\/YUI_EMBED>/gi, "</embed>");
    ...
    F = F.replace(/<script([^>]*)>/gi, "<bad>");
    F = F.replace(/<\/script([^>]*)>/gi, "</bad>");
    F = F.replace(/&lt;script([^>]*)&gt;/gi, "<bad>");
    F = F.replace(/&lt;\/script([^>]*)&gt;/gi, "</bad>");
    F = F.replace(/\n/g, "<YUI_LF>").replace(/\r/g, "<YUI_LF>");
    F = F.replace(new RegExp("<bad([^>]*)>(.*?)</bad>", "gi"), "");
  ...
  }, "cleanHTML": function (H)
  {
    ...
    var G = this.get("markup");
    H = this.pre_filter_linebreaks(H, G);
    ...
    H = this.post_filter_linebreaks(H, G);
    if (G == "xhtml")
      {
    ...
    for (var F in this.invalidHTML)
      {
        if (YAHOO.lang.hasOwnProperty(this.invalidHTML, F))
          {
            if (D.isObject(F) && F.keepContents)
              {
                H = H.replace(new RegExp("<" + F + "([^>]*)>(.*?)</" + F + ">", "gi"), "$1");
              }
            else
              {
                H = H.replace(new RegExp("<" + F + "([^>]*)>(.*?)</" + F + ">", "gi"), "");
              }
          }
      }
    this.fireEvent("cleanHTML", { "type": "cleanHTML", "target": this, "html": H });
    return H;
  }, "filter_invalid_lists": function (F)
  {
    F = F.replace(/<\/li>\n/gi, "</li>");
    F = F.replace(/<\/li><ol>/gi, "</li><li><ol>");
    F = F.replace(/<\/ol>/gi, "</ol></li>");
    F = F.replace(/<\/ol><\/li>\n/gi, "</ol>\n");
    F = F.replace(/<\/li><ul>/gi, "</li><li><ul>");
    F = F.replace(/<\/ul>/gi, "</ul></li>");
    F = F.replace(/<\/ul><\/li>\n/gi, "</ul>\n");
    ...
    return F;
  }, "filter_safari": function (F)
  {
    if (this.browser.webkit)
      {
        F = F.replace(/Apple-style-span/gi, "");
        F = F.replace(/style="line-height: normal;"/gi, "");
        F = F.replace(/<li><\/li>/gi, "");
        F = F.replace(/<li> <\/li>/gi, "");
        F = F.replace(/<li>  <\/li>/gi, "");
        F = F.replace(/<div><\/div>/gi, "");
        F = F.replace(/<div> <\/div>/gi, "");
      }
    return F;
  }, "filter_internals": function (F)
  {
    F = F.replace(/\r/g, "");
    F = F.replace(/<\/?(body|head|html)[^>]*>/gi, "");
    F = F.replace(/<YUI_BR><\/li>/gi, "</li>");
    F = F.replace(/yui-tag-span/gi, "");
    F = F.replace(/yui-tag/gi, "");
    F = F.replace(/yui-non/gi, "");
    F = F.replace(/yui-img/gi, "");
    F = F.replace(/ tag="span"/gi, "");
    F = F.replace(/ class=""/gi, "");
    F = F.replace(/ style=""/gi, "");
    F = F.replace(/ class=" "/gi, "");
    F = F.replace(/ class="  "/gi, "");
    F = F.replace(/ target=""/gi, "");
    F = F.replace(/ title=""/gi, "");
    if (this.browser.ie)
      {
        F = F.replace(/ class= /gi, "");
        F = F.replace(/ class= >/gi, "");
        F = F.replace(/_height="([^>])"/gi, "");
        F = F.replace(/_width="([^>])"/gi, "");
      }
    return F;
  }, "filter_all_rgb": function (J)
  {
    var I = new RegExp("rgb\\s*?\\(\\s*?([0-9]+).*?,\\s*?([0-9]+).*?,\\s*?([0-9]+).*?\\)", "gi");
    var F = J.match(I);
    if (D.isArray(F))
      {
        for (var H = 0; H < F.length; ++H)
          {
            var G = this.filter_rgb(F[H]);
            J = J.replace(F[H].toString(), G);
          }
      }
    return J;
  }, "filter_rgb": function (H)
  {
    if (H.toLowerCase().indexOf("rgb") != -1)
      {
        var K = new RegExp("(.*?)rgb\\s*?\\(\\s*?([0-9]+).*?,\\s*?([0-9]+).*?,\\s*?([0-9]+).*?\\)(.*?)", "gi");
        var G = H.replace(K, "$1,$2,$3,$4,$5").split(",");
        if (G.length == 5)
          {
            var J = parseInt(G[1], 10).toString(16);
            var I = parseInt(G[2], 10).toString(16);
            var F = parseInt(G[3], 10).toString(16);
            J = J.length == 1 ? "0" + J : J;
            I = I.length == 1 ? "0" + I : I;
            F = F.length == 1 ? "0" + F : F;
            H = "#" + J + I + F;
    ...
    return H;
  }, "pre_filter_linebreaks": function (G, F)
  {
    if (this.browser.webkit)
      {
        G = G.replace(/<br class="khtml-block-placeholder">/gi, "<YUI_BR>");
        G = G.replace(/<br class="webkit-block-placeholder">/gi, "<YUI_BR>");
      }
    G = G.replace(/<br>/gi, "<YUI_BR>");
    G = G.replace(/<br (.*?)>/gi, "<YUI_BR>");
    G = G.replace(/<br\/>/gi, "<YUI_BR>");
    G = G.replace(/<br \/>/gi, "<YUI_BR>");
    G = G.replace(/<div><YUI_BR><\/div>/gi, "<YUI_BR>");
    G = G.replace(/<p>(&nbsp;|&#160;)<\/p>/g, "<YUI_BR>");
    G = G.replace(/<p><br>&nbsp;<\/p>/gi, "<YUI_BR>");
    G = G.replace(/<p>&nbsp;<\/p>/gi, "<YUI_BR>");
    G = G.replace(/<YUI_BR>$/, "");
    G = G.replace(/<YUI_BR><\/p>/g, "</p>");
    return G;
  }, "post_filter_linebreaks": function (G, F)
  {
    if (F == "xhtml")
      {
        G = G.replace(/<YUI_BR>/g, "<br />");
      }
    else
      {
        G = G.replace(/<YUI_BR>/g, "<br>");
  ...
  }, "toString": function ()
  {
    ...
    if (this.get && this.get("element_cont"))
      {
        F = "SimpleEditor (#" + this.get("element_cont").get("id") + ")" + (this.get("disabled") ? " Disabled" : "");
      }
    return F;
  } });
  YAHOO.widget.EditorInfo = { "_instances": { }, "window": { }, "panel": null, "getEditorById": function (F)
  {
    if (!YAHOO.lang.isString(F))
      {
        F = F.id;
      }
    if (this._instances[F])
      {
        return this._instances[F];
      }
    return false;
  }, "toString": function ()
  {
    var F = 0;
    for (var G in this._instances)
      {
        F++;
      }
...
(function ()
{
  ...
  YAHOO.extend(YAHOO.widget.Editor, YAHOO.widget.SimpleEditor, { "STR_BEFORE_EDITOR": "This text field can contain stylized text and graphics. To cycle through all formatting options, use the keyboard shortcut Control + Shift + T to place focus on the toolbar and navigate between option heading names. <h4>Common formatting keyboard shortcuts:</h4><ul><li>Control Shift B sets text to bold</li> <li>Control Shift I sets text to italic</li> <li>Control Shift U underlines text</li> <li>Control Shift [ aligns text left</li> <li>Control Shift | centers text</li> <li>Control Shift ] aligns text right</li> <li>Control Shift L adds an HTML link</li> <li>To exit this text editor use the keyboard shortcut Control + Shift + ESC.</li></ul>", "STR_CLOSE_WINDOW": "Close Window", "STR_CLOSE_WINDOW_NOTE": "To close this window use the Control + Shift + W key", "STR_IMAGE_PROP_TITLE": "Image Options", "STR_IMAGE_URL": "Image Url", "STR_IMAGE_TITLE": "Description", "STR_IMAGE_SIZE": "Size", "STR_IMAGE_ORIG_SIZE": "Original Size", "STR_IMAGE_COPY": "<span class=\"tip\"><span class=\"icon icon-info\"></span><strong>Note:</strong>To move this image just highlight it, cut, and paste where ever you'd like.</span>", "STR_IMAGE_PADDING": "Padding", "STR_IMAGE_BORDER": "Border", "STR_IMAGE_TEXTFLOW": "Text Flow", "STR_LOCAL_FILE_WARNING": "<span class=\"tip\"><span class=\"icon icon-warn\"></span><strong>Note:</strong>This image/link points to a file on your computer and will not be accessible to others on the internet.</span>", "STR_LINK_PROP_TITLE": "Link Options", "STR_LINK_PROP_REMOVE": "Remove link from text", "STR_LINK_NEW_WINDOW": "Open in a new window.", "STR_LINK_TITLE": "Description", "CLASS_LOCAL_FILE": "warning-localfile", "CLASS_HIDDEN": "yui-hidden", "init": function (G, F)
  {
    ...
    YAHOO.widget.Editor.superclass.init.call(this, G, F);
  }, "initAttributes": function (F)
  {
    YAHOO.widget.Editor.superclass.initAttributes.call(this, F);
  ...
  }, "_fixNodes": function ()
  {
    ...
    for (var G = 0; G < J.length; ++G)
      {
        if (J[G].getAttribute("href", 2))
          {
            I = J[G].getAttribute("src", 2);
            if (this._isLocalFile(I))
              {
                C.addClass(J[G], this.CLASS_LOCAL_FILE);
              }
            else
              {
                C.removeClass(J[G], this.CLASS_LOCAL_FILE);
              }
          }
      }
    var H = this._getDoc().body.getElementsByTagName("a");
    for (var F = 0; F < H.length; ++F)
      {
        if (H[F].getAttribute("href", 2))
          {
            I = H[F].getAttribute("href", 2);
            if (this._isLocalFile(I))
              {
                C.addClass(H[F], this.CLASS_LOCAL_FILE);
              }
            else
              {
                C.removeClass(H[F], this.CLASS_LOCAL_FILE);
              }
          }
      }
  }, "_disabled": ["createlink","forecolor","backcolor","fontname","fontsize","superscript","subscript","removeformat","heading","indent"], "_alwaysDisabled": { "outdent": true }, "_alwaysEnabled": { "hiddenelements": true }, "_handleKeyDown": function (H)
  {
    YAHOO.widget.Editor.superclass._handleKeyDown.call(this, H);
  ...
  }, "_handleCreateLinkClick": function ()
  {
    ...
    this.on("afterExecCommand", function ()
    {
      var M = new YAHOO.widget.EditorWindow("createlink", { "width": "350px" });
      var J = this.currentElement[0], I = "", P = "", N = "", K = false;
      if (J)
        {
          if (J.getAttribute("href", 2) !== null)
            {
              I = J.getAttribute("href", 2);
              if (this._isLocalFile(I))
                {
                  M.setFooter(this.STR_LOCAL_FILE_WARNING);
                  K = true;
                }
              else
                {
          ...
          if (J.getAttribute("title") !== null)
            {
              P = J.getAttribute("title");
            }
          if (J.getAttribute("target") !== null)
            {
              N = J.getAttribute("target");
            }
        }
      var O = "<label for=\"createlink_url\"><strong>" + this.STR_LINK_URL + ":</strong> <input type=\"text\" name=\"createlink_url\" id=\"createlink_url\" value=\"" + I + "\"" + (K ? " class=\"warning\"" : "") + "></label>";
      O += "<label for=\"createlink_target\"><strong>&nbsp;</strong><input type=\"checkbox\" name=\"createlink_target_\" id=\"createlink_target\" value=\"_blank\"" + (N ? " checked" : "") + "> " + this.STR_LINK_NEW_WINDOW + "</label>";
      O += "<label for=\"createlink_title\"><strong>" + this.STR_LINK_TITLE + ":</strong> <input type=\"text\" name=\"createlink_title\" id=\"createlink_title\" value=\"" + P + "\"></label>";
      var L = document.createElement("div");
      L.innerHTML = O;
      var H = document.createElement("div");
      H.className = "removeLink";
      var G = document.createElement("a");
      G.href = "#";
      G.innerHTML = this.STR_LINK_PROP_REMOVE;
      G.title = this.STR_LINK_PROP_REMOVE;
      A.on(G, "click", function (Q)
      {
        A.stopEvent(Q);
      ...
      H.appendChild(G);
      L.appendChild(H);
      M.setHeader(this.STR_LINK_PROP_TITLE);
      M.setBody(L);
      A.onAvailable("createlink_url", function ()
      {
        window.setTimeout(function ()
        {
          try 
        ...
        A.on("createlink_url", "blur", function ()
        {
  ...
  {
    var H = C.get("createlink_url"), J = C.get("createlink_target"), L = C.get("createlink_title"), I = this.currentElement[0], F = I;
    if (H && H.value)
      {
        var K = H.value;
        ...
        if (J.checked)
          {
            I.setAttribute("target", J.value);
          }
        else
          {
            I.setAttribute("target", "");
          }
        I.setAttribute("title", L.value ? L.value : "");
      }
    else
      {
        var G = this._getDoc().createElement("span");
        G.innerHTML = I.innerHTML;
        C.addClass(G, "yui-non");
  ...
  }, "_handleInsertImageClick": function ()
  {
    ...
    this.on("afterExecCommand", function ()
    {
      var I = this.currentElement[0], S = null, P = "", e = "", f = "", O = "", b = "", V = 75, Y = 75, U = 0, Q = 0, N = 0, W = false, M = new YAHOO.widget.EditorWindow("insertimage", { "width": "415px" });
      if (!I)
        {
      ...
      if (I)
        {
          if (I.getAttribute("src"))
            {
              O = I.getAttribute("src", 2);
              if (O.indexOf(this.get("blankimage")) != -1)
                {
                  O = this.STR_IMAGE_HERE;
                  W = true;
                }
            }
          if (I.getAttribute("alt", 2))
            {
              f = I.getAttribute("alt", 2);
            }
          if (I.getAttribute("title", 2))
            {
              f = I.getAttribute("title", 2);
            }
          if (I.parentNode && this._isElement(I.parentNode, "a"))
            {
              P = I.parentNode.getAttribute("href", 2);
              if (I.parentNode.getAttribute("target") !== null)
                {
                  e = I.parentNode.getAttribute("target");
                }
            }
          V = parseInt(I.height, 10);
          Y = parseInt(I.width, 10);
          if (I.style.height)
            {
              V = parseInt(I.style.height, 10);
            }
          if (I.style.width)
            {
              Y = parseInt(I.style.width, 10);
            }
          if (I.style.margin)
            {
              U = parseInt(I.style.margin, 10);
            }
          if (!I._height)
            {
              I._height = V;
      ...
      var Z = "<label for=\"insertimage_url\"><strong>" + this.STR_IMAGE_URL + ":</strong> <input type=\"text\" id=\"insertimage_url\" value=\"" + O + "\" size=\"40\"></label>";
      S = document.createElement("div");
      S.innerHTML = Z;
      var K = document.createElement("div");
      K.id = "img_toolbar";
      S.appendChild(K);
      var F = "<label for=\"insertimage_title\"><strong>" + this.STR_IMAGE_TITLE + ":</strong> <input type=\"text\" id=\"insertimage_title\" value=\"" + f + "\" size=\"40\"></label>";
      F += "<label for=\"insertimage_link\"><strong>" + this.STR_LINK_URL + ":</strong> <input type=\"text\" name=\"insertimage_link\" id=\"insertimage_link\" value=\"" + P + "\"></label>";
      F += "<label for=\"insertimage_target\"><strong>&nbsp;</strong><input type=\"checkbox\" name=\"insertimage_target_\" id=\"insertimage_target\" value=\"_blank\"" + (e ? " checked" : "") + "> " + this.STR_LINK_NEW_WINDOW + "</label>";
      ...
      var G = "0";
      var X = "solid";
      if (I.style.borderLeftWidth)
        {
          G = parseInt(I.style.borderLeftWidth, 10);
        }
      if (I.style.borderLeftStyle)
        {
          X = I.style.borderLeftStyle;
        }
      var d = H.getButtonByValue("bordersize");
      var a = parseInt(G, 10) > 0 ? "" : "none";
      d.set("label", "<span class=\"yui-toolbar-bordersize-" + G + "\">" + a + "</span>");
      this._updateMenuChecked("bordersize", G, H);
      var R = H.getButtonByValue("bordertype");
      R.set("label", "<span class=\"yui-toolbar-bordertype-" + X + "\"></span>");
      ...
      var J = H.get("cont");
      var c = document.createElement("div");
      c.className = "yui-toolbar-group yui-toolbar-group-height-width height-width";
      c.innerHTML = "<h3>" + this.STR_IMAGE_SIZE + ":</h3>";
      var L = "";
      if (V != Q || Y != N)
        {
          L = "<span class=\"info\">" + this.STR_IMAGE_ORIG_SIZE + "<br>" + N + " x " + Q + "</span>";
        }
      c.innerHTML += "<span><input type=\"text\" size=\"3\" value=\"" + Y + "\" id=\"insertimage_width\"> x <input type=\"text\" size=\"3\" value=\"" + V + "\" id=\"insertimage_height\"></span>" + L;
      J.insertBefore(c, J.firstChild);
      A.onAvailable("insertimage_width", function ()
      {
        A.on("insertimage_width", "blur", function ()
        {
          var g = parseInt(C.get("insertimage_width").value, 10);
          if (g > 5)
            {
              I.style.width = g + "px";
              this.moveWindow();
      ...
      A.onAvailable("insertimage_height", function ()
      {
        A.on("insertimage_height", "blur", function ()
        {
          var g = parseInt(C.get("insertimage_height").value, 10);
          if (g > 5)
            {
              I.style.height = g + "px";
              this.moveWindow();
      ...
      if (I.align == "right" || I.align == "left")
        {
          H.selectButton(I.align);
        }
      else
        {
          if (I.style.display == "block")
            {
              H.selectButton("block");
            }
          else
            {
              H.selectButton("inline");
            }
        }
      if (parseInt(I.style.marginLeft, 10) > 0)
        {
          H.getButtonByValue("padding").set("label", "" + parseInt(I.style.marginLeft, 10));
        }
      if (I.style.borderSize)
        {
          H.selectButton("bordersize");
          H.selectButton(parseInt(I.style.borderSize, 10));
        }
      H.on("colorPickerClicked", function (k)
      {
        var h = "1", j = "solid", g = "black";
        if (I.style.borderLeftWidth)
          {
            h = parseInt(I.style.borderLeftWidth, 10);
          }
        if (I.style.borderLeftStyle)
          {
            j = I.style.borderLeftStyle;
          }
        if (I.style.borderLeftColor)
          {
            g = I.style.borderLeftColor;
          }
        var i = h + "px " + j + " #" + k.color;
        I.style.border = i;
      }, this.toolbar, true);
      H.on("buttonClick", function (m)
      {
        var k = m.button.value, j = "";
        if (m.button.menucmd)
          {
            k = m.button.menucmd;
          }
        var h = "1", i = "solid", g = "black";
        if (I.style.borderLeftWidth)
          {
            h = parseInt(I.style.borderLeftWidth, 10);
          }
        ...
        switch (k) {
          case "bordersize":
            ...
            j = parseInt(m.button.value, 10) + "px " + i + " " + g;
            I.style.border = j;
            if (parseInt(m.button.value, 10) > 0)
              {
          ...
          case "block":
            ...
            break;
          case "padding":
            var l = H.getButtonById(m.button.id);
            I.style.margin = l.get("label") + "px";
            break;
        }
        H.selectButton(m.button.value);
        this.moveWindow();
      }, this, true);
      M.setHeader(this.STR_IMAGE_PROP_TITLE);
      M.setBody(S);
      if (this.browser.webkit && !this.browser.webkit3 || this.browser.opera)
        {
          M.setFooter(this.STR_IMAGE_COPY);
        }
      this.openWindow(M);
      A.onAvailable("insertimage_url", function ()
      {
        ...
        if (this.get("localFileWarning"))
          {
            ...
            {
              var g = C.get("insertimage_link");
              if (this._isLocalFile(g.value))
                {
            ...
            A.on("insertimage_url", "blur", function ()
            {
              ...
              if (this._isLocalFile(i.value))
                {
                  C.addClass(i, "warning");
                  this.get("panel").setFooter(this.STR_LOCAL_FILE_WARNING);
                }
              else
                {
                  if (this.currentElement[0])
                    {
                      C.removeClass(i, "warning");
                      this.get("panel").setFooter(" ");
                      if (this.browser.webkit && !this.browser.webkit3 || this.browser.opera)
                        {
                          this.get("panel").setFooter(this.STR_IMAGE_COPY);
                        }
                      if (i && i.value && i.value != this.STR_IMAGE_HERE)
                        {
                          this.currentElement[0].setAttribute("src", i.value);
                          var h = this, g = new Image();
                          g.onerror = function ()
                          {
                            i.value = h.STR_IMAGE_HERE;
                            g.setAttribute("src", h.get("blankimage"));
                            h.currentElement[0].setAttribute("src", h.get("blankimage"));
                            YAHOO.util.Dom.get("insertimage_height").value = g.height;
                            YAHOO.util.Dom.get("insertimage_width").value = g.width;
                          };
                          window.setTimeout(function ()
                          {
                            YAHOO.util.Dom.get("insertimage_height").value = g.height;
                            YAHOO.util.Dom.get("insertimage_width").value = g.width;
                            if (h.currentElement && h.currentElement[0])
                              {
                                if (!h.currentElement[0]._height)
                                  {
                                    h.currentElement[0]._height = g.height;
                                  }
                                if (!h.currentElement[0]._width)
                                  {
                                    h.currentElement[0]._width = g.width;
                                  }
                              }
                            h.moveWindow();
                          }, 200);
                          if (i.value != this.STR_IMAGE_HERE)
                            {
                              g.src = i.value;
                            }
                        }
  ...
  {
    var F = C.get("insertimage_url");
    var M = C.get("insertimage_title");
    var J = C.get("insertimage_link");
    var K = C.get("insertimage_target");
    var I = this.currentElement[0];
    if (F && F.value && F.value != this.STR_IMAGE_HERE)
      {
        I.setAttribute("src", F.value);
        I.setAttribute("title", M.value);
        I.setAttribute("alt", M.value);
        var H = I.parentNode;
        if (J.value)
          {
            var L = J.value;
            if (L.indexOf("://") == -1 && L.substring(0, 1) != "/" && L.substring(0, 6).toLowerCase() != "mailto")
              {
                if (L.indexOf("@") != -1 && L.substring(0, 6).toLowerCase() != "mailto")
                  {
                    L = "mailto:" + L;
                  }
                else
            ...
            if (H && this._isElement(H, "a"))
              {
                H.setAttribute("href", L);
                if (K.checked)
                  {
                    H.setAttribute("target", K.value);
                  }
                else
                  {
              ...
              {
                var G = this._getDoc().createElement("a");
                G.setAttribute("href", L);
                if (K.checked)
                  {
                    G.setAttribute("target", K.value);
                  }
                else
                  {
  ...
  }, "_renderPanel": function ()
  {
    ...
    if (!YAHOO.widget.EditorInfo.panel)
      {
        F = new YAHOO.widget.Overlay(this.EDITOR_PANEL_ID, { "width": "300px", "iframe": true, "visible": false, "underlay": "none", "draggable": false, "close": false });
        YAHOO.widget.EditorInfo.panel = F;
      }
    else
      {
        F = YAHOO.widget.EditorInfo.panel;
      }
    this.set("panel", F);
  ...
  {
    this.toolbar.set("disabled", true);
    A.on(document, "keypress", this._closeWindow, this, true);
    if (YAHOO.widget.EditorInfo.window.win && YAHOO.widget.EditorInfo.window.scope)
      {
        YAHOO.widget.EditorInfo.window.scope.closeWindow.call(YAHOO.widget.EditorInfo.window.scope);
      }
    YAHOO.widget.EditorInfo.window.win = K;
    ...
    this.fireEvent("beforeOpenWindow", { "type": "beforeOpenWindow", "win": K, "panel": N });
    M = document.createElement("div");
    M.className = this.CLASS_PREFIX + "-body-cont";
    for (var V in this.browser)
      {
        if (this.browser[V])
          {
            C.addClass(M, V);
    ...
    C.addClass(M, YAHOO.widget.Button && this._defaultToolbar.buttonType == "advanced" ? "good-button" : "no-button");
    var S = document.createElement("h3");
    S.className = "yui-editor-skipheader";
    S.innerHTML = this.STR_CLOSE_WINDOW_NOTE;
    M.appendChild(S);
    form = document.createElement("form");
    form.setAttribute("method", "GET");
    var W = K.name;
    A.on(form, "submit", function (Y)
    {
      var X = "window" + W + "Submit";
      P.fireEvent(X, { "type": X, "target": this });
      A.stopEvent(Y);
    }, this, true);
    M.appendChild(form);
    if (D.isObject(K.body))
      {
        form.appendChild(K.body);
      }
    else
      {
        var F = document.createElement("div");
    ...
    I.innerHTML = K.header;
    N.cfg.setProperty("width", K.attrs.width);
    N.setHeader(" ");
    N.appendToHeader(I);
    I.appendChild(J);
    I.appendChild(H);
    ...
    if (K.footer !== null)
      {
        N.setFooter(K.footer);
        C.addClass(N.footer, "open");
      }
    else
      {
        C.removeClass(N.footer, "open");
      }
    N.appendToBody(M);
    var Q = function ()
    {
      N.bringToTop();
      A.on(N.element, "click", function (X)
      {
        A.stopPropagation(X);
      });
      this._setBusy(true);
      N.showEvent.unsubscribe(Q);
    };
    N.showEvent.subscribe(Q, this, true);
    var G = function ()
    {
      this.currentWindow = null;
      var X = "window" + W + "Close";
      this.fireEvent(X, { "type": X, "target": this });
      N.hideEvent.unsubscribe(G);
    };
    N.hideEvent.subscribe(G, this, true);
    this.currentWindow = K;
    this.moveWindow(true);
    N.show();
    this.fireEvent("afterOpenWindow", { "type": "afterOpenWindow", "win": K, "panel": N });
  }, "moveWindow": function (G)
  {
    if (!this.currentWindow)
      {
        return false;
      }
    var J = this.currentWindow, K = C.getXY(this.currentElement[0]), b = C.getXY(this.get("iframe").get("element")), P = this.get("panel"), Z = [K[0] + b[0],K[1] + b[1]], S = parseInt(J.attrs.width, 10) / 2, V = "center", R = P.cfg.getProperty("xy"), H = J._knob, Y = 0, M = 0, U = false;
    Z[0] = Z[0] - S + 20;
    Z[0] = Z[0] - C.getDocumentScrollLeft(this._getDoc());
    Z[1] = Z[1] - C.getDocumentScrollTop(this._getDoc());
    if (this._isElement(this.currentElement[0], "img"))
      {
        if (this.currentElement[0].src.indexOf(this.get("blankimage")) != -1)
          ...
          {
            var O = parseInt(this.currentElement[0].width, 10);
            var X = parseInt(this.currentElement[0].height, 10);
            Z[0] = Z[0] + O / 2;
            Z[1] = Z[1] + X;
          }
        Z[1] = Z[1] + 15;
      ...
      {
        var L = C.getStyle(this.currentElement[0], "fontSize");
        if (L && L.indexOf && L.indexOf("px") != -1)
          {
            Z[1] = Z[1] + parseInt(C.getStyle(this.currentElement[0], "fontSize"), 10) + 5;
          }
        else
          {
            Z[1] = Z[1] + 20;
    ...
    if (Z[0] + S * 2 > b[0] + parseInt(this.get("iframe").get("element").clientWidth, 10))
      {
        Z[0] = b[0] + parseInt(this.get("iframe").get("element").clientWidth, 10) - S * 2 - 5;
        V = "right";
      }
    try 
    {
    ...
    var Q = b[1] + parseInt(this.get("height"), 10);
    ...
    if (Y > 10 || M > 10 || G)
      {
        ...
        if (this.currentElement[0].width)
          {
            W = parseInt(this.currentElement[0].width, 10) / 2;
          }
        var N = K[0] + b[0] + W;
        T = N - Z[0];
        if (T > parseInt(J.attrs.width, 10) - 1)
          {
            T = parseInt(J.attrs.width, 10) - 1;
          }
        else
          {
            if (T < 40)
        ...
        if (G)
          {
            ...
            if (this.get("animate"))
              {
                C.setStyle(P.element, "opacity", "0");
                U = new YAHOO.util.Anim(P.element, { "opacity": { "from": 0, "to": 1 } }, 0.1, YAHOO.util.Easing.easeOut);
                P.cfg.setProperty("xy", Z);
        ...
        else
          {
            if (this.get("animate"))
              {
                U = new YAHOO.util.Anim(P.element, { }, 0.5, YAHOO.util.Easing.easeOut);
                U.attributes = { "top": { "to": Z[1] }, "left": { "to": Z[0] } };
                U.onComplete.subscribe(function ()
                {
                  P.cfg.setProperty("xy", Z);
                });
                var a = new YAHOO.util.Anim(P.iframe, U.attributes, 0.5, YAHOO.util.Easing.easeOut);
                var F = new YAHOO.util.Anim(H, { "left": { "to": T } }, 0.6, YAHOO.util.Easing.easeOut);
                U.animate();
                a.animate();
                F.animate();
              }
  ...
  }, "_closeWindow": function (F)
  {
    if (F.charCode == 87 && F.shiftKey && F.ctrlKey)
      {
        if (this.currentWindow)
          {
            this.closeWindow();
  ...
  }, "closeWindow": function ()
  {
    ...
    this.toolbar.resetAllButtons();
    this._focusWindow();
    A.removeListener(document, "keypress", this._closeWindow);
  }, "cmd_heading": function (J)
  {
    var G = true, H = null, I = "heading", K = this._getSelection(), F = this._getSelectedElement();
    if (F)
      {
        K = F;
      }
    ...
    if (J == "none")
      {
        if (K && K.tagName && K.tagName.toLowerCase().substring(0, 1) == "h" || K && K.parentNode && K.parentNode.tagName && K.parentNode.tagName.toLowerCase().substring(0, 1) == "h")
          {
            if (K.parentNode.tagName.toLowerCase().substring(0, 1) == "h")
              {
                K = K.parentNode;
              }
            ...
            H = this._swapEl(F, "span", function (L)
            {
              L.className = "yui-non";
            });
            this._selectNode(H);
      ...
      {
        if (this._isElement(F, "h1") || this._isElement(F, "h2") || this._isElement(F, "h3") || this._isElement(F, "h4") || this._isElement(F, "h5") || this._isElement(F, "h6"))
          {
            H = this._swapEl(F, J);
            this._selectNode(H);
            this.currentElement[0] = H;
          ...
          {
            this._createCurrentElement(J);
            this._selectNode(this.currentElement[0]);
          }
        G = false;
      }
    return [G,I];
  }, "cmd_hiddenelements": function (F)
  {
    ...
    if (this.browser.webkit && !this._getDoc().queryCommandEnabled("removeformat"))
      {
        ...
        this.currentElement[0].innerHTML = F;
        for (var H = 1; H < this.currentElement.length; ++H)
          {
            this.currentElement[H].parentNode.removeChild(this.currentElement[H]);
          }
        G = false;
      }
    return [G];
  }, "cmd_script": function (L, K)
  {
    var H = true, F = L.toLowerCase().substring(0, 3), I = null, G = this._getSelectedElement();
    if (this.browser.webkit)
      {
        if (this._isElement(G, F))
          {
            I = this._swapEl(this.currentElement[0], "span", function (M)
            {
              M.className = "yui-non";
            });
            this._selectNode(I);
    ...
    return H;
  }, "cmd_superscript": function (F)
  {
    return [this.cmd_script("superscript", F)];
  }, "cmd_subscript": function (F)
  {
    return [this.cmd_script("subscript", F)];
  }, "cmd_indent": function (I)
  {
    var F = true, H = this._getSelectedElement(), J = null;
    if (this.browser.webkit || this.browser.ie || this.browser.gecko)
      {
        if (this._isElement(H, "blockquote"))
          {
            J = this._getDoc().createElement("blockquote");
        ...
        else
          {
            ...
            for (var G = 0; G < this.currentElement.length; ++G)
              {
                J = this._getDoc().createElement("blockquote");
                J.innerHTML = this.currentElement[G].innerHTML;
                this.currentElement[G].parentNode.replaceChild(J, this.currentElement[G]);
                this.currentElement[G] = J;
              }
            this._selectNode(this.currentElement[0]);
          }
        F = false;
    ...
    return [F,"indent",I];
  }, "cmd_outdent": function (J)
  {
    var F = true, I = this._getSelectedElement(), K = null, G = null;
    if (this.browser.webkit || this.browser.ie || this.browser.gecko)
      {
        I = this._getSelectedElement();
        if (this._isElement(I, "blockquote"))
          {
            var H = I.parentNode;
            if (this._isElement(I.parentNode, "blockquote"))
              {
            ...
            else
              {
                G = this._getDoc().createElement("span");
                G.innerHTML = I.innerHTML;
                YAHOO.util.Dom.addClass(G, "yui-non");
                H.replaceChild(G, I);
                this._selectNode(G);
              }
          }
  ...
  }, "toString": function ()
  {
    ...
    if (this.get && this.get("element_cont"))
      {
        F = "Editor (#" + this.get("element_cont").get("id") + ")" + (this.get("disabled") ? " Disabled" : "");
      }
    return F;
  } });
  YAHOO.widget.EditorWindow = function (G, F)
  {
    this.name = G.replace(" ", "_");
    this.attrs = F;
  };
  YAHOO.widget.EditorWindow.prototype = { "_cache": null, "header": null, "body": null, "footer": null, "setHeader": function (F)
  {
    this.header = F;
  }, "setBody": function (F)
  {
...
YAHOO.register("editor", YAHOO.widget.Editor, { "version": "2.5.0", "build": "897" });
YAHOO.namespace("Lifestyles.ImageUploader");
YAHOO.Lifestyles.ImageUploader = function (el, fields, oArgs)
{
  var button;
  this.prefix = oArgs.prefix || "ylf-";
  ...
  this.wrapper = this.fileList.parentNode.parentNode;
  this.flashwrapper = this.fileList.parentNode;
  YAHOO.util.Dom.addClass(this.fileList, "files");
  YAHOO.util.Event.addListener(this.fileList, "click", this.dispatchClick, this, true);
  this.uploader = new YAHOO.widget.Uploader(oArgs.elUploader);
  this.uploader.subscribe("contentReady", this.contentDone, this, true);
  ...
  if (this.totalUploadedCount.length > 0)
    {
      this.highestStepReached = 4;
    }
  else
    {
  ...
  buttons.id = this.prefix + "buttons";
  for (var m in this.modes)
    {
      if (YAHOO.lang.hasOwnProperty(this.modes, m))
        {
          if (!this.lastMode)
            {
              this.setMode(m);
            }
          if (this.modes[m].hidden)
            {
              continue;
            }
          button = document.createElement("button");
          button.id = this.prefix + m + "-btn";
          button.name = button.className = m;
          if (this.highestStepReached < this.modes[m].step)
            {
              button.className += " disabled";
              button.disabled = true;
            }
          button.innerHTML = (this.mode == "stepped" ? ++i + ". " : "") + this.modes[m].title;
          YAHOO.util.Event.addListener(button, "click", function (e, uploader)
          {
            uploader.setMode(this.name, true);
          }, this);
  ...
  buttons = button = null;
  if (oArgs.useFileHeader)
    {
      this.addRow(this.fields, true);
    }
  if (this.highestStepReached == 4)
    {
      this.setMode("upload");
      this.finalizeUpload(true);
    }
};
YAHOO.Lifestyles.ImageUploader.prototype = { "NEXT_MODE": "next_mode", "PREV_MODE": "prev_mode", "getTemplateMarkup": function (elId)
{
  var el = YAHOO.util.Dom.get(elId);
  return el ? el.innerHTML.replace("<!--!", "").replace("!-->", "") : "";
}, "addRow": function (oData, asHeader)
{
  var col, row, key;
  var listNum = 1;
  ...
  for (var f in this.fields)
    {
      if (YAHOO.lang.hasOwnProperty(this.fields, f))
        {
          col = document.createElement("div");
          key = this.fields[f].key;
          col.className = "field " + key;
          col.innerHTML = asHeader ? this.fields[f].label : oData[key];
          row.appendChild(col);
        }
    }
  if (asHeader)
    {
      row.className = "header cls";
      row.id = this.prefix + "file-header";
  ...
  else
    {
      ...
      col.innerHTML = this.actionsTpl;
      row.appendChild(col);
      row.className = "file cls";
      YAHOO.util.Dom.generateId(row, this.prefix + "file-");
      this.fileList.appendChild(row);
      if (row.previousSibling)
        {
          listNum = parseInt(row.previousSibling.firstChild.innerHTML, 10) + 1;
        }
      row.innerHTML = "<div class=\"list\">" + listNum + "</div>" + row.innerHTML;
    }
  return row.id;
}, "updateRow": function (rowId, oData)
{
  var row = YAHOO.util.Dom.get(rowId);
  if (!row)
    {
      return false;
    }
  for (var f in oData)
    {
      if (YAHOO.lang.hasOwnProperty(oData, f))
        {
          var col = YAHOO.util.Dom.getElementsByClassName(f, "div", row);
          if (col.length == 1)
            {
              col[0].innerHTML = oData[f];
            }
        }
    }
  return true;
}, "dispatchClick": function (e)
{
  ...
  if (YAHOO.util.Dom.hasClass(target, "move"))
    {
      this.moveRow(e);
    }
  else
    if (YAHOO.util.Dom.hasClass(target, "delete"))
      {
        this.deleteRow(e);
      }
}, "deleteRow": function (e, rowid)
{
  var row;
  if (rowid)
    {
      row = YAHOO.util.Dom.get(rowid);
      row.parentNode.removeChild(row);
    }
  else
    {
      row = YAHOO.util.Dom.getAncestorByClassName(YAHOO.util.Event.getTarget(e), "file");
      var confirmDeleteSSRow = new YAHOO.Lifestyles.PopUpConfirm();
      confirmDeleteSSRow.yesHandler = function ()
      {
        row.parentNode.removeChild(row);
        this.confirmDialog.hide();
        YAHOO.Lifestyles.Uploader.finalizeUpload();
        YAHOO.Lifestyles.Uploader.reNumber();
      };
      confirmDeleteSSRow.init("delete", { "header": "Delete This Photo", "body": "Are you sure you would like to delete this photo from your gallery?" });
      confirmDeleteSSRow.confirmDialog.show();
    }
}, "reNumber": function (e)
{
  var getList = YAHOO.util.Dom.getElementsByClassName("list", "div", this.fileList);
  for (var i = 0, len = getList.length; i < len; ++i)
    {
      getList[i].innerHTML = i + 1;
    }
}, "cancelUpload": function ()
{
  var confirmCanelUploadSS = new YAHOO.Lifestyles.PopUpConfirm();
  confirmCanelUploadSS.yesHandler = function (e, uploader)
  {
    var loading = YAHOO.util.Dom.getElementsByClassName("loading", "img", "ylf-files");
    for (var i = 0, len = loading.length; i < len; ++i)
      {
        YAHOO.Lifestyles.Uploader.deleteRow(YAHOO.Lifestyles.Uploader, loading[i].parentNode.parentNode.id);
      }
    var unloaded = YAHOO.util.Dom.getElementsByClassName("placeholder", "img", "ylf-files");
    for (var i = 0, len = unloaded.length; i < len; ++i)
      {
        YAHOO.Lifestyles.Uploader.deleteRow(YAHOO.Lifestyles.Uploader, unloaded[i].parentNode.parentNode.id);
      }
    this.confirmDialog.hide();
    YAHOO.Lifestyles.Uploader.finalizeUpload(true);
    YAHOO.Lifestyles.Uploader.reNumber();
    this.groupCount = 0;
    YAHOO.Lifestyles.Uploader.uploader.cancel();
  };
  confirmCanelUploadSS.init("delete", { "header": "Cancel This Upload", "body": "Are you sure you would like to cancel the upload in progress?" });
  confirmCanelUploadSS.confirmDialog.show();
}, "moveRow": function (e)
{
  var target = YAHOO.util.Event.getTarget(e);
  if (YAHOO.util.Dom.hasClass(target, "move"))
    {
      ...
      if (YAHOO.util.Dom.hasClass(target, "up"))
        {
          this.fileList.insertBefore(row, YAHOO.util.Dom.getPreviousSibling(row));
        }
      else
        if (YAHOO.util.Dom.hasClass(target, "down"))
          {
            if (row == this.fileList.lastChild)
              {
                YAHOO.util.Dom.insertBefore(row, this.fileList.firstChild);
              }
            else
              {
                YAHOO.util.Dom.insertAfter(row, YAHOO.util.Dom.getNextSibling(row));
              }
          }
      this.reNumber();
      var nodeAnim = new YAHOO.util.Anim(row, { "opacity": { "from": 0, "to": 1 } }, 0.7, YAHOO.util.Easing.easeOut);
      nodeAnim.animate();
    }
...
{
  var fileInfo;
  if (this.lastMode != "upload")
    {
      this.setMode("upload");
    }
  document.getElementById(this.prefix + "upload-btn").disabled = true;
  this.completedCount = this.groupCount = 0;
  for (var i in event.fileList)
    {
      if (this.totalUploadedCount.length + this.groupCount + 1 <= this.maxUpload)
        {
          fileInfo = { "fileId": event.fileList[i].id, "size": event.fileList[i].size, "details": "<div class=\"filename\">" + event.fileList[i].name + "</div>", "thumb": this.thumbStart, "progress": "In Queue" };
          fileInfo.rowId = this.addRow(fileInfo);
          this.filesInProgress[fileInfo.fileId] = { "rowId": fileInfo.rowId, "filename": event.fileList[i].name };
          this.filesInProgress.length++;
          if (this.groupCount === 0)
            {
              if (YAHOO.util.Dom.get("statusWrapper"))
                {
                  YAHOO.util.Event.purgeElement("statusWrapper", true);
                  this.flashwrapper.removeChild(YAHOO.util.Dom.get("statusWrapper"));
                }
              statusDiv = document.createElement("div");
              statusDiv.id = "statusWrapper";
              this.flashwrapper.insertBefore(statusDiv, this.flashwrapper.firstChild);
              statusDiv.innerHTML = "<div id=\"statusOf\">Starting Upload...</div><div id=\"" + this.statusBarId + "\"><div id=\"" + this.statusBarChildId + "\"></div></div><button type=\"button\" id=\"" + this.cancelBtnId + "\">" + this.cancelBtnText + "</button>";
              YAHOO.util.Event.on(this.cancelBtnId, "click", function (e, uploader)
              {
  ...
  YAHOO.util.Dom.addClass(this.wrapper, "uploading");
  this.upload();
}, "upload": function (id)
{
  this.uploader.setSimUploadLimit(4);
  this.uploader.uploadAll("/api/post/photo/upload", "POST", { "title": "image", "alias": this.alias, "u": document.cookie, "crb": this.crb }, "image");
}, "startRow": function (event)
{
  this.updateRow(this.filesInProgress[event.id].rowId, { "progress": "Starting...", "thumb": this.thumbProgress });
}, "updateProgress": function (event)
{
  ...
  this.updateRow(this.filesInProgress[event.id].rowId, { "progress": progbar });
}, "processResponse": function (event)
{
  var errorCode;
  var photo_info = { };
  photo_info.rowId = this.filesInProgress[event.id].rowId;
  photo_info.filename = this.filesInProgress[event.id].filename;
  var xml = this.stringToDom(event.data);
  try 
  {
    errorCode = xml.getElementsByTagName("error")[0].getAttribute("uploadErrorCode");
  }
  catch (err)
  {
    errorCode = "0";
  ...
  if (respStatus === "1" && fileStatus === "1" && errorCode === "0")
    {
      var photo = xml.getElementsByTagName("photo_url")[0];
      photo_info.id = photo.getAttribute("photo_id");
      photo_info.thumb = photo.getElementsByTagName("url_thumb")[0].firstChild.nodeValue;
      photo_info.original = photo.getElementsByTagName("url_original")[0].firstChild.nodeValue;
      photo_info.width = xml.getElementsByTagName("ImageWidth")[0].firstChild.nodeValue;
      photo_info.height = xml.getElementsByTagName("ImageHeight")[0].firstChild.nodeValue;
      var completedPercentage = Math.round((this.completedCount + 1) / this.groupCount * 100) + "%";
      YAHOO.util.Dom.setStyle(this.statusBarChildId, "width", completedPercentage);
      YAHOO.util.Dom.get("statusOf").innerHTML = this.completedCount + 1 + " of " + this.groupCount + " photos uploaded";
    ...
    {
      this.handleError(event, errorCode);
    }
  this.finalizeUpload();
}, "finalizeUpload": function (cancel)
{
  var uploadcount;
  if (++this.completedCount >= this.groupCount || cancel)
    {
      YAHOO.util.Dom.get(this.prefix + "upload-btn").disabled = false;
      YAHOO.util.Dom.addClass(this.wrapper, "files-uploaded");
      YAHOO.util.Dom.removeClass(this.wrapper, "uploading");
      ...
      this.filesInProgress = { };
      this.filesInProgress.length = 0;
      if (this.maxUpload - this.totalUploadedCount.length >= 0)
        {
          uploadcount = this.maxUpload - this.totalUploadedCount.length;
        }
      else
        {
          uploadcount = 0;
        }
      if (this.maxUpload == uploadcount)
        {
          this.setMode("intro");
        }
      else
        {
          var statusWrapperEl = YAHOO.util.Dom.get("statusWrapper");
          if (!statusWrapperEl)
            {
              var newDiv = document.createElement("div");
              newDiv.id = "statusWrapper";
              statusWrapperEl = this.flashwrapper.insertBefore(newDiv, this.flashwrapper.firstChild);
            }
          statusWrapperEl.innerHTML = "You can <button id=\"" + this.uploadMoreBtnId + "\" type=\"button\">Upload " + uploadcount + " More Photo(s)</button>";
          if (uploadcount === 0)
            {
              statusWrapperEl.innerHTML = this.maxUploadError;
            }
        }
    }
}, "stripTags": function (e)
{
  ...
  if (target.nodeName.toLowerCase() !== "input")
    ...
    {
      var tempvalue = target.value.replace(/(<([^>]+)>)/ig, "");
      target.value = tempvalue;
    }
}, "toggleDetails": function (e, args)
{
  for (var i = 1, len = args.fields.length; i < len; ++i)
    {
      YAHOO.util.Dom.removeClass(args.fields[i], "hide");
      YAHOO.util.Dom.addClass(args.fields[i], "show");
    }
  if (this.lastFocusedRow && this.lastFocusedRow != args.id)
    {
      var fieldsToHide = YAHOO.util.Dom.getElementsByClassName("show", "div", this.lastFocusedRow);
      YAHOO.util.Dom.batch(fieldsToHide, function (el)
      {
        YAHOO.util.Dom.removeClass(el, "show");
        YAHOO.util.Dom.addClass(el, "hide");
      });
    }
  this.lastFocusedRow = args.id;
}, "handleError": function (event, errorCode)
{
  var newError, fileValue, temprowid;
  if (event.status > 399)
    {
      if (errorCode == "1" || event.status == 413)
        {
          newError = this.maxSizeUploadError;
        }
      else
        {
          newError = this.errorTpl;
        }
      try 
      {
        temprowid = this.filesInProgress[event.id].rowId;
        fileValue = YAHOO.util.Dom.getElementsByClassName("filename", "div", temprowid);
        fileValue = fileValue[0].innerHTML;
      }
      catch (err)
      {
        fileValue = "";
      ...
      this.updateRow(this.filesInProgress[event.id].rowId, { "progress": "Error", "thumb": this.thumbStart, "details": newError });
      this.finalizeUpload();
    }
}, "processMode": function (mode)
{
  var button;
  if (mode && this.lastMode != mode)
    {
      YAHOO.util.Dom.replaceClass(this.wrapper, this.lastMode, mode);
      if (this.modes[mode].step > this.highestStepReached)
        {
          this.highestStepReached = this.modes[mode].step;
        }
      ...
      for (var m in this.modes)
        {
          if (YAHOO.lang.hasOwnProperty(this.modes, m))
            {
              button = YAHOO.util.Dom.get(this.prefix + m + "-btn");
              if (button && button.disabled && this.modes[m].step <= this.highestStepReached)
                {
...
}, "setMode": function (mode, clicked)
{
  var lastLoopMode, useModeOnNextLoop;
  if (this.lockWhileUploading && this.filesInProgress.length)
    {
      return;
    }
      ...
      for (var m in this.modes)
        {
          if (YAHOO.lang.hasOwnProperty(this.modes, m))
            {
              if (m == mode || useModeOnNextLoop)
                {
                  if (!(this.modes[m].delayed && clicked) || useModeOnNextLoop)
                    {
                      this.processMode(m);
                      mode = m;
                    }
                ...
                if (!this.modes[mode])
                  {
                    if (this.NEXT_MODE === mode && this.lastMode == m)
                      {
                        useModeOnNextLoop = true;
                      }
                    else
                      if (this.PREV_MODE === mode && this.lastMode == m)
                        {
                          this.processMode(lastLoopMode);
                          mode = lastLoopMode;
                        }
                  }
              lastLoopMode = m;
...
}, "contentDone": function ()
{
  ...
  var ff = [{ "description": "Gallery Images (*.jpg, *.gif)", "extensions": "*.jpeg;*.jpg;*.gif" }];
  this.uploader.setFileFilters(ff);
}, "initMetadata": function ()
{
  YAHOO.util.Event.addListener(this.prefix + "files", "mouseout", this.stripTags);
}, "initDrag": function ()
{
  new YAHOO.util.DDTarget(this.fileList.id);
  var items = YAHOO.util.Dom.getElementsByClassName("file", "div", this.fileList);
  for (var i = 0, len = items.length; i < len; ++i)
    {
      new YAHOO.Lifestyles.ImageUploader.File(items[i].id);
    }
}, "lockDrag": function ()
{
...
YAHOO.Lifestyles.ImageUploader.File = function (id, sGroup, config)
{
  YAHOO.Lifestyles.ImageUploader.File.superclass.constructor.call(this, id, sGroup, config);
  this.lastNode = this.lastNodeUp = this.lastNodeDown = null;
  this.goingUp = false;
  this.lastY = 0;
};
YAHOO.extend(YAHOO.Lifestyles.ImageUploader.File, YAHOO.util.DDProxy, { "startDrag": function (x, y)
{
  var dragEl = this.getDragEl();
  dragEl.className = "proxy";
  var clickEl = this.getEl();
  YAHOO.util.Dom.addClass(clickEl, "dragged");
}, "onDrag": function (e)
{
...
}, "onDragOver": function (e, id)
{
  ...
  if (YAHOO.util.Dom.hasClass(destEl, "file"))
    {
      this.lastNode = destEl;
      if (this.goingUp)
        {
          this.setDragIndicators(destEl, "above");
          this.lastNodeUp = destEl;
          this.lastNodeDown = null;
        }
      else
        {
          this.setDragIndicators(destEl, "below");
...
{
  this.setDragIndicators();
  var srcEl = this.getEl();
  var proxy = this.getDragEl();
  var p = srcEl.parentNode;
  if (this.lastNode == this.lastNodeUp)
    {
      p.insertBefore(srcEl, this.lastNode);
    }
  else
    if (this.lastNode == this.lastNodeDown)
      {
        p.insertBefore(srcEl, this.lastNode.nextSibling);
      }
  YAHOO.util.Dom.removeClass(srcEl, "dragged");
  YAHOO.util.Dom.removeClass(proxy, "proxy-hide");
  var a = new YAHOO.util.Motion(proxy, { "points": { "to": YAHOO.util.Dom.getXY(srcEl) } }, 0.2, YAHOO.util.Easing.easeOut);
  a.onComplete.subscribe(function ()
  {
    YAHOO.util.Dom.addClass(proxy, "proxy-hide");
  });
  a.animate();
  YAHOO.Lifestyles.Uploader.reNumber();
}, "setDragIndicators": function (el, elClass)
{
  YAHOO.util.Dom.removeClass(this.lastNodeUp, "above");
  YAHOO.util.Dom.removeClass(this.lastNodeDown, "below");
  if (el && elClass)
    {
      YAHOO.util.Dom.addClass(el, elClass);
    }
} });
YAHOO.Lifestyles.RTE = function ()
{
  var myConfig = { "height": "300px", "width": "630px", "animate": true, "dompath": true, "focusAtStart": true, "extracss": "img { display:block; margin:0 auto; }  html { width:628px; overflow:hidden; }  .yui-media { display:block; height:100px; width:100px; overflow:hidden; border:1px solid #bbb; cursor:pointer !important; background:#f2f2f2 url(http://l.yimg.com/jn/images/20090129191543/media_placeholder.gif) no-repeat center center; }  .yui-media * { position:relative; left:-999em; }",
 "toolbar": { "buttons": [{ "group": "textstyle", "label": "Font Style", "buttons": [{ "type": "push", "label": "Bold CTRL + SHIFT + B", "value": "bold" },
{ "type": "push", "label": "Italic CTRL + SHIFT + I", "value": "italic" },
{ "type": "push", "label": "Strike Through", "value": "strikethrough" },
{ "type": "push", "label": "Remove Formatting", "value": "removeformat", "disabled": true }] },
{ "type": "separator" },
{ "group": "indentlist", "label": "Lists", "buttons": [{ "type": "push", "label": "Create an Unordered List", "value": "insertunorderedlist" },
{ "type": "push", "label": "Create an Ordered List", "value": "insertorderedlist" }] },
{ "type": "separator" },
{ "group": "insertitem", "label": "Insert Item", "buttons": [{ "type": "push", "label": "Highlight words to add a link", "value": "createlink", "disabled": false },
{ "type": "push", "label": "Image Options", "value": "insertimage" },
{ "type": "push", "label": "Media Options", "value": "insertmedia" }] },
{ "type": "separator" }] }] } };
  var myEditor = new YAHOO.widget.Editor("editor", myConfig);
  var Dom = YAHOO.util.Dom, Event = YAHOO.util.Event, Lang = YAHOO.lang;
  var blgChannel = Dom.get("channel"), blgEditor = Dom.get("editor"), blgTitle = Dom.get("blog_title"), schMonth = Dom.get("scheduledMonth"), schDay = Dom.get("scheduledDay"), schYear = Dom.get("scheduledYear"), schHour = Dom.get("scheduledHour"), schMins = Dom.get("scheduledMinutes");
  var isContentModified = false;
  var crumb = Dom.get("crb").value;
  ...
  var setContentModifiedFlag = function ()
  {
    isContentModified = true;
    Event.removeListener(contentFields, "keyup");
    Event.removeListener(contentFields, "change");
  };
  var contentFields = [blgTitle,blgChannel,blgEditor,"blog_tags","excerpt"];
  Event.on(contentFields, "keyup", function ()
  {
    this.blur();
    this.focus();
  });
  Event.on(contentFields, "change", setContentModifiedFlag, null, true);
  myEditor.on("editorKeyPress", function checkIsEditorDirty()
  {
    if (myEditor.editorDirty === true)
      {
        setContentModifiedFlag();
        myEditor.removeListener("editorKeyPress", checkIsEditorDirty);
      }
  }, null, true);
  var _getEmbedEl = function ()
  {
    embedCode = Dom.get("embed-code").value;
    if (YAHOO.env.ua.ie)
      {
        var startembed = embedCode.indexOf("<embed");
        var endembed = embedCode.indexOf("</embed>");
        embedCode = embedCode.substring(startembed, endembed) + "</embed>";
      }
    var tempDivEl = document.createElement("div");
    tempDivEl.innerHTML = embedCode;
    var embedEl = tempDivEl.getElementsByTagName("embed")[0];
    var newEmbedEl = document.createElement("embed");
    newEmbedEl.setAttribute("src", embedEl.src);
    newEmbedEl.setAttribute("width", embedEl.width);
    newEmbedEl.setAttribute("height", embedEl.height);
    newEmbedEl.setAttribute("flashVars", embedEl.getAttribute("flashvars") || "");
    newEmbedEl.setAttribute("allowfullscreen", embedEl.allowfullscreen || "true");
    newEmbedEl.setAttribute("type", "application/x-shockwave-flash");
    return newEmbedEl;
  };
  var _handleMediaWindow = function ()
  {
    var editorWindow = new YAHOO.widget.EditorWindow("insertmedia", { "width": "425px" });
    var el = this._getSelectedElement();
    var embedContent = "";
    if (el.nodeName.toLowerCase() === "span" && Dom.hasClass(el, "yui-media"))
      {
        embedContent = el.innerHTML;
      }
    var bodyText = "<p><em>Want to add a video to your blog?</em> If it is hosted on another site, it will be easy to add to your blog.  Just paste the <strong>Embed</strong> code into the box below.</p>  <div class=\"embed-container\">   <div id=\"embed-preview\"><span>Video Preview</span></div>   <textarea id=\"embed-code\" rows=\"8\" cols=\"25\">" + embedContent + "</textarea>  </div>  <dl>   <dt><span>*</span> Yahoo! Video Users:</dt>   <dd>Click on the <strong>envelope icon</strong> at the bottom of the video. The <strong>Embed</strong> code will appear above.</dd>   <dt><span>*</span> Flickr Video Users:</dt>   <dd>Click on <strong>\u00e2\u20ac\u0153Share This\u00e2\u20ac\u009d</strong> at the top right of the screen, and then click on <strong>\u00e2\u20ac\u0153Embed it\u00e2\u20ac\u009d</strong> in the box that appears.</dd>   <dt><span>*</span> YouTube Users:</dt>   <dd>The <strong>Embed</strong> code is to the right of the video, inside the information box.</dd>  </dl>  <div id=\"doneCont\" class=\"cls\"><button id=\"mediaDone\">Done</button><button id=\"previewBtn\">Preview</button></div>";
    editorWindow.setHeader("Edit Media");
    editorWindow.setBody(bodyText);
    this.openWindow(editorWindow);
    Dom.setStyle("yui-editor-panel", "margin-top", "85px");
    Event.on("mediaDone", "click", function ()
    {
      Dom.setStyle("yui-editor-panel", "margin-top", "0");
      myEditor.closeWindow();
    });
    Event.on("previewBtn", "click", function ()
    {
      var previewEl = Dom.get("embed-preview");
      try 
      {
        var embedEl = _getEmbedEl();
        embedEl.setAttribute("width", "160");
        embedEl.setAttribute("height", "140");
        embedEl.setAttribute("wmode", "");
        if (YAHOO.env.ua.ie)
          {
            var tempDivEl = document.createElement("div");
            tempDivEl.appendChild(embedEl);
            previewEl.innerHTML = tempDivEl.innerHTML;
          }
        else
          {
            previewEl.innerHTML = "";
            previewEl.appendChild(embedEl);
          }
      }
      catch (ex)
      {
        previewEl.innerHTML = "<span>Please make sure you\u00e2\u20ac\u2122ve copied over the correct <strong>Embed</strong> code.</span>";
      }
      this.moveWindow();
      this.get("panel").syncIframe();
    }, this, true);
  ...
  myEditor.on("windowinsertmediaClose", function ()
  {
    ...
    try 
    {
      ...
      if (parseInt(embedEl.getAttribute("width"), 10) > 600)
        {
          embedEl.setAttribute("width", "600");
        }
      if (this.browser.ie && embedEl.outerHTML)
        {
          el.innerHTML = embedEl.outerHTML;
        }
      else
        {
          el.innerHTML = "";
  ...
  var resize = function ()
  {
    var editorWrapperEl = this.get("editor_wrapper");
    var editorEl = Dom.get("editor_editor");
    var iframeDoc = this._getDoc();
    var height = Dom.getStyle(editorWrapperEl, "height");
    var pxHeight = this.browser.ie ? iframeDoc.body.scrollHeight : iframeDoc.documentElement.scrollHeight;
    var newHeight = pxHeight + "px";
    if (height != newHeight && pxHeight >= "300" && Dom.getStyle(editorEl, "position") !== "absolute")
      {
        Dom.setStyle(editorWrapperEl, "height", newHeight);
        if (this.browser.ie)
          {
            Dom.setStyle(editorEl, "height", newHeight);
            Dom.setStyle(editorEl, "overflow", "hidden");
          }
      }
  };
  ...
  function yuiImgUploader(rte, upload_url, upload_image_name)
  {
    ...
    {
      rte.toolbar.on("insertimageClick", function (o)
      {
        try 
        {
          ...
          imgPanel.on("contentReady", function ()
          {
            try 
            {
              while (!Dom.get("imageDone"))
                {
                  Dom.setStyle(Dom.get("img_toolbar").nextSibling, "display", "none");
                  var img_elem = Dom.get("insertimage_url");
                  var imgHeight = Dom.get("insertimage_height");
                  var imgLink = Dom.get("insertimage_link");
                  var filediv = document.createElement("div");
                  filediv.id = "fileUploadField";
                  filediv.innerHTML = "<p><span>*</span> We accept <em>jpg</em> and <em>gif</em> images with a <em>4MB</em> max file upload size</p><div class=\"menuTitle\">Add an image from your computer</div><div class=\"indentMenu\"><span>Step 1.</span> Click Browse to select an image on your computer<br /><form method=\"post\" enctype=\"multipart/form-data\" id=\"aForm\"><input type=\"hidden\" value=\"" + crumb + "\" name=\"crb\"><input type=\"hidden\" name=\"title\" value=\"User Image\" /><strong>File:</strong><input type=\"file\" id=\"insertimage_upload\"  name=\"image\" size=\"27\" /></label><input type=\"hidden\" size=\"10\" name=\"alias\" value=\"" + usrAlias + "\"></form></div><div id=\"loadingArea\"></div><div class=\"indentMenu\"><span>Step 2.</span> And then Upload it&nbsp; <button id=\"insertimage_upload_btn\">Upload</button></div><div style=\"clear:right;\" class=\"menuTitle\">Or Add an image from the web</div>";
                  Dom.insertBefore(filediv, img_elem.parentNode);
                  var editPhoto = document.createElement("div");
                  editPhoto.innerHTML = "Edit Your Image";
                  editPhoto.className = "menuTitle";
                  Dom.insertAfter(editPhoto, img_elem.parentNode);
                  var optDiv = document.createElement("div");
                  optDiv.innerHTML = "<br /><a href=\"#\" id=\"imageOptions\">More edit options \u00c2\u00bb</a>";
                  Dom.insertAfter(optDiv, imgHeight.parentNode.parentNode.parentNode);
                  var doneDiv = document.createElement("div");
                  doneDiv.id = "doneCont";
                  doneDiv.innerHTML = "<button id=\"imageDone\">Done</button>";
                  Dom.insertAfter(doneDiv, Dom.get("img_toolbar").nextSibling);
                  var linkDiv = document.createElement("div");
                  linkDiv.innerHTML = "Want to add a link to your image?";
                  linkDiv.className = "menuTitle";
                  Dom.insertBefore(linkDiv, imgLink.parentNode);
                  Event.on("imageDone", "click", function (ev)
                  {
                    myEditor.closeWindow();
                  });
                  Event.on("imageOptions", "click", function (ev)
                  {
                  ...
                  Event.on("insertimage_upload_btn", "click", function (ev)
                  {
                    Dom.setStyle(Dom.get("loadingArea").nextSibling, "display", "none");
                    ...
                    if (fileUpl !== "" && (fileExt === "jpg" || fileExt === "gif" || fileExtWindows === "jpeg"))
                      {
                        Event.stopEvent(ev);
                        var callback = { "upload": function (o)
                        {
                          var xml = o.responseXML;
                          try 
                          {
                            var medium = xml.getElementsByTagName("url_medium")[0].firstChild.nodeValue;
                            ...
                            if (height > width)
                              {
                                if (height < 400)
                                  {
                                    Dom.get("insertimage_url").value = original;
                                  }
                                else
                                  {
                                    Dom.get("insertimage_url").value = medium;
                                  }
                              }
                            else
                              {
                            ...
                            Dom.get("loadingArea").innerHTML = "";
                            Dom.setStyle(Dom.get("fileUploadField"), "display", "none");
                            Dom.setStyle(Dom.get("fileUploadField").nextSibling, "display", "none");
                            var uploadConfirm = document.createElement("div");
                            uploadConfirm.innerHTML = "<p>Your image has been uploaded successfully!</p>";
                            Dom.insertAfter(uploadConfirm, Dom.get("fileUploadField"));
                            Dom.setStyle(Dom.get("fileUploadField").nextSibling.nextSibling, "display", "none");
                            Dom.get("img_toolbar").previousSibling.innerHTML = "Want to edit your image?";
                          }
                          catch (e)
                          {
                            Dom.get("loadingArea").innerHTML = "There was an error uploading your image. Please make sure your image meets the upload requirements.";
                            Dom.setStyle(Dom.get("loadingArea").nextSibling, "display", "block");
                          }
                        } };
                        Dom.get("loadingArea").innerHTML = "It could take up to 5 minutes for your image to upload.<br><img src=\"http://l.yimg.com/jn/images/20090129191543/ajax-loader2.gif\"><br /><button id=\"insertimage_cancel_upload_btn\">Cancel Upload</button>";
                        var formObject = Dom.get("aForm");
                        YAHOO.util.Connect.setForm(formObject, true);
						/* Photo Upload uri */
                        var cObj = YAHOO.util.Connect.asyncRequest("POST", "/api/post/photo/upload", callback);
                        Event.on("insertimage_cancel_upload_btn", "click", function ()
                        {
                          YAHOO.util.Connect.abort(cObj);
                          Dom.get("loadingArea").innerHTML = "Image upload has been cancelled.";
                          Dom.setStyle(Dom.get("loadingArea").nextSibling, "display", "block");
                        });
                        return false;
                      }
                      ...
                      if (fileUpl === "")
                        {
                          Dom.get("loadingArea").innerHTML = "Please Select a File to Upload.";
                          Dom.setStyle(Dom.get("loadingArea").nextSibling, "display", "block");
                        }
                      else
                        {
                          Dom.get("loadingArea").innerHTML = "Please upload an image with a \"gif\" or \"jpg\" file format.";
                          Dom.setStyle(Dom.get("loadingArea").nextSibling, "display", "block");
                        }
                  });
                }
  ...
  function handleLinkWindow(rte)
  {
    rte.on("toolbarLoaded", function ()
    {
      rte.toolbar.on("createlinkClick", function (o)
      {
        try 
        {
          var lnkPanel = new YAHOO.util.Element("yui-editor-panel");
          lnkPanel.on("contentReady", function ()
          {
            var linkDiv = document.createElement("div");
            linkDiv.innerHTML = "<button id=\"linkDone\">Done</button>";
            var lnk_elem = Dom.get("createlink_title");
            linkDiv.id = "linkDoneCon";
            Dom.insertAfter(linkDiv, lnk_elem.parentNode.nextSibling);
            YAHOO.util.Event.on("linkDone", "click", function ()
            {
              rte.closeWindow();
            });
          });
  ...
  myEditor.on("toolbarLoaded", function ()
  {
    var htmlbtn = Dom.get("htmlMode");
    var rtebtn = Dom.get("rteMode");
    Dom.removeClass(htmlbtn, "off");
    Dom.removeClass(rtebtn, "off");
    var RTEMode = function (e)
    {
      Event.preventDefault(e);
      var ta = myEditor.get("element");
      var iframe = myEditor.get("iframe").get("element");
      var target = Event.getTarget(e).id;
      if (target === "rteMode")
        {
          myEditor.toolbar.set("disabled", false);
          Dom.addClass(rtebtn, "rteOnSelect");
          Dom.removeClass(htmlbtn, "rteOnSelect");
        ...
        {
          Dom.addClass(htmlbtn, "rteOnSelect");
          Dom.removeClass(rtebtn, "rteOnSelect");
          myEditor.cleanHTML();
          Dom.addClass(iframe, "editor-hidden");
          Dom.removeClass(ta, "editor-hidden");
          myEditor.toolbar.set("disabled", true);
          myEditor.dompath.innerHTML = "Editing HTML Code";
          myEditor.hide();
        }
    };
    Event.on("rteMode", "click", RTEMode);
    Event.on("htmlMode", "click", RTEMode);
  ...
  myEditor.render();
  yuiImgUploader(myEditor, "/file_upload.php", "");
  handleLinkWindow(myEditor);
  var previewBlog = function ()
  {
    var orgstr;
    var cidImage = "http://l.yimg.com/a/i/identity/nopic_32.gif";
    var avatar = Dom.get("avatar");
    if (avatar)
      {
        if (avatar.getAttribute("longdesc"))
          {
            cidImage = avatar.getAttribute("longdesc");
          }
        else
          {
            cidImage = avatar.getAttribute("src");
          }
      }
    if (Dom.get("excerpt").value === "")
      {
        Dom.setStyle("previewExcerpt", "display", "none");
        Dom.setStyle("yourPost", "display", "none");
      }
    else
      {
        Dom.setStyle("previewExcerpt", "display", "block");
        Dom.setStyle("yourPost", "display", "block");
        Dom.get("excerpt_content_preview").innerHTML = "<img src=\"http://l.yimg.com/jn/images/20090129191543/ajax-loader2.gif\" style=\"margin: 0px auto\">";
      }
    Dom.get("blog_content_preview").innerHTML = "<img src=\"http://l.yimg.com/jn/images/20090129191543/ajax-loader2.gif\" style=\"margin: 0px auto\">";
    var blogTags = Dom.get("blog_tags").value;
    var relatedLinks = Dom.getElementsByClassName("related", "p", "ylf-blogpost")[0];
    if (blogTags === "")
      {
        Dom.setStyle(relatedLinks, "display", "none");
      }
    else
      {
        Dom.setStyle(relatedLinks, "display", "block");
        Dom.get("blog_tags_preview").innerHTML = blogTags;
      }
    Dom.get("blog_title_preview").innerHTML = Dom.get("blog_title").value;
    Dom.get("cidAuth2").innerHTML = Dom.get("nickName") ? Dom.get("nickName").innerHTML : "you";
    Dom.get("cidImage2").setAttribute("src", cidImage);
    Dom.setStyle("previewBlog", "display", "block");
    Dom.setStyle("composeBlog", "display", "none");
    Dom.setStyle("edit", "display", "inline");
    ...
    if (thumbs.length > 0)
      {
        ...
        if (parseInt(orgHeight, 10) >= parseInt(orgWidth, 10))
          {
            orgstr = "http://l.yimg.com/jn/util/anysize/500y-86400," + escape(original);
          }
        else
          {
            orgstr = "http://l.yimg.com/jn/util/anysize/630*500l-86400," + escape(original);
          }
        var looplen = thumbs.length > 7 ? 7 : thumbs.length;
        var thumbstr = "";
        var thumbclass = "";
        for (var i = 0; i < looplen; ++i)
          {
            thumbclass = i === 0 ? " class= \"yltasis active\"" : " class= \"yltasis\"";
        ...
        galleryHTML = "<div class=\"post-gallery\" class=\"cls\"><div id=\"ypv-thumbs\" class=\"hide\"><div class=\"controls\"><a class=\"prev yltasis\"><span>Prev</span></a> <a class=\"next yltasis\"><span>Next</span></a><div class=\"navigation\">photos <span id=\"navRange\">1 - " + looplen + "</span> of " + thumbs.length + "</div></div><div class=\"viewer\"><ul class=\"list\"><li class=\"item\"><ul>" + thumbstr + "<li></li><li></li><li></li><li></li><li></li><li></li></ul></li></ul></div></div><div id=\"ypv-current\"><div id=\"ypv-photo\"><div id=\"photo-nav\">photo <span id=\"photo-count\">1</span> of " + thumbs.length + "</div><div id=\"photo-cont\"><div class=\"photo\"><img src=\"" + orgstr + "\" /></div></div><div id=\"photo-info\" class=\"cls\"><h4 id=\"photoTitle\">" + titless + "</h4><p class=\"caption\" id=\"photoCaption\">" + desc + "</p><cite id=\"captionCite\">" + credit + "</cite></div></div></div></div>";
        if (Dom.get("sstop").checked)
          {
            Dom.get("blog_gallery_preview_top").innerHTML = galleryHTML;
          }
        else
          {
    ...
    var cObject = YAHOO.util.Connect.asyncRequest("POST", "/blog-post-preview/", callback);
    Dom.get("p").focus();
  };
  var editBlog = function ()
  {
    Dom.setStyle("previewBlog", "display", "none");
    Dom.setStyle("composeBlog", "display", "block");
    Dom.setStyle("edit", "display", "none");
    Dom.setStyle("preview", "display", "inline");
    Dom.get("preview").focus();
  };
  var resizeImageSetSize = function ()
  {
    var elmHeight, elmWidth;
    var iframeBody = myEditor._getDoc().body;
    var imgs = iframeBody.getElementsByTagName("img");
    for (var i = 0; i < imgs.length; ++i)
      {
        elmHeight = imgs[i].offsetHeight;
        elmWidth = imgs[i].offsetWidth;
        if (elmWidth > 570 && elmWidth > elmHeight)
          {
            elmHeight = Math.round(elmHeight * (575 / elmWidth));
            elmWidth = 575;
          }
        else
          if (elmHeight > 575 && elmHeight > elmWidth)
            {
              elmWidth = Math.round(elmWidth * (575 / elmHeight));
              elmHeight = 575;
            }
  ...
  var displayAdvOpt = function (e)
  {
    Event.preventDefault(e);
    if (Dom.getStyle("adv_opt_dis", "display") === "none")
      {
        Dom.setStyle("adv_opt_dis", "display", "block");
        Dom.get("excerpt").focus();
      }
    else
      {
        Dom.setStyle("adv_opt_dis", "display", "none");
      }
  };
  var displayAdvOptCopy = function (e)
  {
    Event.preventDefault(e);
    saveRTE();
    Dom.get("excerpt").value = myEditor.get("textarea").value;
    Dom.get("excerpt").focus();
  };
  var buttonSwitch = function (e)
  {
    ...
    if (Dom.getStyle("editor_editor", "position") !== "absolute")
      {
        saveRTE();
      }
    ...
    Dom.removeClass("scheduled", "error");
    var hasError = false;
    var markError = function (focPt)
    {
      Dom.addClass(focPt, "error");
      hasError = true;
    };
    if (blgTitle.value === "")
      {
    ...
    if (schMonth && (schMonth.value !== "" || schDay.value !== "" || schYear.value !== "" || schHour.value !== "" || schMins.value !== "") && (schMonth.value === "" || schDay.value === "" || schYear.value === "" || schHour.value === "" || schMins.value === ""))
      {
        markError("scheduled");
      }
    if (hasError)
      {
        ...
        errorArea.innerHTML = "<img src=\"http://l.yimg.com/jn/images/20090129191543/error.gif\"> <span>Please make sure the required fields are filled in:</span>";
    ...
    if (target.id === "preview")
      ...
      if (target.id === "edit" || target.id === "edit2")
        {
          editBlog();
        }
      else
        if (Dom.hasClass(target, "post"))
          {
  ...
  window.onbeforeunload = function ()
  {
    ...
    if (isContentModified === true)
      {
        return "You have unsaved content which will be lost if you leave this page now.";
      }
  };
}();